    ...
    method lower($ir_func) {
        ...
        for my $block ( $ir_func->blocks->@* ) {
            ...
            if ( $ir_func->blocks->[0] == $block && $ir_func->params->@* ) {
                ...
                my $frame_base_vreg;
                my $ensure_frame_base = sub ($mbb) {
                    return $frame_base_vreg if defined $frame_base_vreg;
                    $frame_base_vreg = '%__frame_base';
                    my $dst = Brocken::Jenny::MIR::MachineOperand->new(
                        kind  => 'virt_reg',
                        value => $frame_base_vreg,
                        type  => Brocken::Lindsay::IR::Type::i64()
                    );
                    my $src = Brocken::Jenny::MIR::MachineOperand->new( kind => 'phys_reg', value => 'rbp' );
                    $mbb->add_instruction(
                        Brocken::Jenny::MIR::MachineInstruction->new(
                            opcode   => 'mov',
                            operands => [ $dst, $src ],
                            comment  => 'capture rbp for stack param access'
                        )
                    );
                    return $frame_base_vreg;
                };
                for ( my $i = 0; $i <= $last; $i++ ) {
                    ...
                    if ($is_i128) {
                        ...
                        if ( $gp_idx + 1 < @gp_regs ) {
                            ...
                            $mbb->add_instruction(
                                Brocken::Jenny::MIR::MachineInstruction->new(
                                    opcode   => 'load',
                                    operands => [ $lo_tmp, $mem_lo ],
                                    comment  => "save param $i lo from stack"
                                )
                            );
                            $mbb->add_instruction(
                                Brocken::Jenny::MIR::MachineInstruction->new(
                                    opcode   => 'load',
                                    operands => [ $hi_tmp, $mem_hi ],
                                    comment  => "save param $i hi from stack"
                                )
                            );
                        }
                    }
                ...
                for ( my $i = 0; $i <= $last; $i++ ) {
                    ...
                    if ($is_i128) {
                        ...
                        my $tmp
                            = Brocken::Jenny::MIR::MachineOperand->new( kind => 'virt_reg', value => $param_name . '.entry', type => $param->type );
                        $mbb->add_instruction(
                            Brocken::Jenny::MIR::MachineInstruction->new(
            ...
            for my $inst ( $block->instructions->@* ) {
                ...
                if ( $opcode eq 'add' ||
                    ...
                    if (
                        ...
                        if ($is_float) {
                            ...
                            $mbb->add_instruction(
                                Brocken::Jenny::MIR::MachineInstruction->new(
                                    opcode   => 'mov',
                                    operands => [ $tmp, $self->_lower_opnd_wide( $lhs, $inst->type ) ],
                        ...
                        if ( $src->kind eq 'imm' ) {
                            if ( $src->value == 0 ) {
                                $mbb->add_instruction(
                                    Brocken::Jenny::MIR::MachineInstruction->new(
                                        opcode   => 'mov',
                                        operands => [ $lo, Brocken::Jenny::MIR::MachineOperand->new( kind => 'imm', value => 0 ) ],
                                        comment  => 'i128 zext lo = 0'
                                    )
                                );
                            }
                            else {
                                my $tmp = Brocken::Jenny::MIR::MachineOperand->new( kind => 'virt_reg', value => $inst->name . '_tmp',
                                    type => $val->type );
                                $mbb->add_instruction(
                                    Brocken::Jenny::MIR::MachineInstruction->new(
                                        opcode   => 'mov',
                                        operands => [ $tmp, $src ],
                                        comment  => 'i128 zext lo materialize imm'
                                    )
                                );
                                $mbb->add_instruction(
                                    Brocken::Jenny::MIR::MachineInstruction->new(
                                        opcode   => 'movzx',
                                        operands => [ $lo, $tmp ],
                                        comment  => 'i128 zext lo from ' . ( $val->name || $val->value )
                                    )
                                );
                            }
                        }
                        ...
                        if ( $src->kind eq 'imm' ) {
                            ...
                            $mbb->add_instruction(
                                Brocken::Jenny::MIR::MachineInstruction->new(
                                    opcode   => 'mov',
                                    operands => [ $tmp, $src ],
                                    comment  => 'i128 sext materialize imm'
                                )
                            );
                            $mbb->add_instruction(
                                Brocken::Jenny::MIR::MachineInstruction->new(
                                    opcode   => 'movsx',
                                    operands => [ $lo, $tmp ],
                                    comment  => 'i128 sext lo from ' . ( $val->name || $val->value )
                                )
                            );
                        }
                        else {
                    ...
                    if ($is_i128) {
                        ...
                        if ( $val->type && $val->type->kind eq 'float' ) {
                            ...
                            if ( $val->isa('Brocken::Lindsay::IR::RodataRef') ) {
                                ...
                                my $val_opnd = $self->_lower_opnd_wide( $val, $store_type );
                    ...
                    if ( $inst->type && $inst->type->kind eq 'float' ) {
                        ...
                        if ( $inst->type && $inst->type->bits == 128 ) {
                            ...
                            $mbb->add_instruction(
                                Brocken::Jenny::MIR::MachineInstruction->new(
                                    opcode   => 'sub',
                                    operands => [ $dst_lo, $lo_src ],
                                    comment  => 'i128 neg: lo -= val_lo'
                                )
                            );
                            $mbb->add_instruction(
                                Brocken::Jenny::MIR::MachineInstruction->new(
                                    opcode   => 'mov',
                                    operands => [ $dst_hi, $zero ],
                            ...
                            $mbb->add_instruction(
                                Brocken::Jenny::MIR::MachineInstruction->new(
                                    opcode   => 'sbb',
                                    operands => [ $dst_hi, $hi_src ],
                                    comment  => 'i128 neg: hi -= val_hi (with borrow)'
                                )
                            );
                        }
                        else {
                    ...
                    my $mem_header = Brocken::Jenny::MIR::MachineOperand->new(
                        kind  => 'mem',
                    ...
                    for my $i ( reverse 0 .. $#args ) {
                        ...
                        if ( ref $reg && $reg->[0] eq 'stack' ) {
                            if ($is_i128) {
                                ...
                                $mbb->add_instruction(
                                    Brocken::Jenny::MIR::MachineInstruction->new(
                                        opcode   => $stack_op,
                                        operands => [ $mem, $val ],
                                        comment  => "arg $i to stack"
                                    )
                                );
                            }
                        }
                        elsif ($is_i128) {
                            my ( $lo_reg_name, $hi_reg_name ) = $reg->@*;
                            my $lo_reg = Brocken::Jenny::MIR::MachineOperand->new( kind => 'phys_reg', value => $lo_reg_name );
                            my $hi_reg = Brocken::Jenny::MIR::MachineOperand->new( kind => 'phys_reg', value => $hi_reg_name );
                            my ( $lo, $hi ) = $self->_split_i128( $args[$i] );
                            $mbb->add_instruction(
                            ...
                            my $reg_name = $reg;
                            my $reg_op   = Brocken::Jenny::MIR::MachineOperand->new( kind => 'phys_reg', value => $reg_name );
                            if ( $args[$i]->isa('Brocken::Lindsay::IR::RodataRef') ) {
                                $mbb->add_instruction(
                                    Brocken::Jenny::MIR::MachineInstruction->new(
                                        opcode   => 'lea_rodata',
                                        operands => [
                                            $reg_op, Brocken::Jenny::MIR::MachineOperand->new( kind => 'rodata_label', value => $args[$i]->label )
                                        ],
                    ...
                    elsif ( $platform->is_linux || $platform->is_freebsd ) {
