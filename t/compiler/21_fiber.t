...
subtest 'Fiber keyword parsing' => sub {
    ...
    my $ast    = $parser->parse();
    ok $ast->isa('Brocken::AST::Program'), 'parsed program';
    is scalar(@{$ast->stmts}), 1, 'one statement';
    ok $ast->stmts->[0]->isa('Brocken::AST::Async::FiberBlock'), 'statement is FiberBlock';
    my $fb = $ast->stmts->[0];
    ok $fb->body->isa('Brocken::AST::Block'), 'fiber body is Block';
    is scalar(@{$fb->body->stmts}), 1, 'one statement in fiber body';
    ok $fb->body->stmts->[0]->isa('Brocken::AST::Async::Yield'), 'body statement is Yield';
    ok defined $fb->body->stmts->[0]->expr, 'yield has expression';
    ok $fb->body->stmts->[0]->expr->isa('Brocken::AST::IntLiteral'), 'yield expr is IntLiteral';
    is $fb->body->stmts->[0]->expr->value, 1, 'yield value is 1';
};
...
subtest 'Fiber with print and yield' => sub {
    ...
    my $selector = Brocken::Compiler::InstructionSelector->new(
        arch => $arch,
        mapping => $mapping,
        emitter => $emitter,
        os => $host_os
...
subtest 'Yield keyword parsing at statement level' => sub {
    ...
    ok $ast->isa('Brocken::AST::Program'), 'parsed program';
    is scalar(@{$ast->stmts}), 1, 'one statement';
    ok $ast->stmts->[0]->isa('Brocken::AST::Async::Yield'), 'statement is Yield';
    ok defined $ast->stmts->[0]->expr, 'yield has expression';
    is $ast->stmts->[0]->expr->value, 42, 'yield value is 42';
};
...
subtest 'Yield without expression' => sub {
    ...
    ok $ast->isa('Brocken::AST::Program'), 'parsed program';
    ok $ast->stmts->[0]->isa('Brocken::AST::Async::Yield'), 'statement is Yield';
    ok !defined $ast->stmts->[0]->expr, 'yield has no expression';
};
