...
subtest 'LinearScan basic allocation' => sub {
    ...
    $bb->add_instruction(
        Brocken::Jenny::MIR::MachineInstruction->new( opcode => 'add', operands => [ $vreg_a, $imm_1 ] )
    );
    $bb->add_instruction(
        Brocken::Jenny::MIR::MachineInstruction->new( opcode => 'add', operands => [ $vreg_b, $vreg_a ] )
    );
    $bb->add_instruction(
...
subtest 'LinearScan with many virtual registers' => sub {
    ...
    for my $i ( 1 .. 20 ) {
        ...
        push @vregs, $vr;
        $bb->add_instruction(
            Brocken::Jenny::MIR::MachineInstruction->new( opcode => 'add', operands => [ $vr, $imm ] )
        );
    }
    $bb->add_instruction(
        Brocken::Jenny::MIR::MachineInstruction->new( opcode => 'ret', operands => [] )
