...
class Brocken::Jenny::Codegen::X86_64 {
    ...
    method emit_functions($ir_funcs) {
        ...
        # Emit gate dispatch trampoline for bind gate support
        if ($has_gate) {
    ...
    # Check if an IR function contains gate dispatch calls
    ...
    method _build_gate_dispatch_mf() {
        ...
        $mbb->add_instruction(
            Brocken::Jenny::MIR::MachineInstruction->new(
                opcode   => 'mov',
                operands => [ $icb, Brocken::Jenny::MIR::MachineOperand->new( kind => 'phys_reg', value => 'rdi' ) ],
                comment  => 'ICB = rdi'
            )
        );
        ...
        $mbb->add_instruction(
            Brocken::Jenny::MIR::MachineInstruction->new(
                opcode   => 'sub',
                operands => [ $rsp, $eight ],
                comment  => 'allocate stack slot for saved ICB'
            )
        );
        my $saved_icb_mem = Brocken::Jenny::MIR::MachineOperand->new( kind => 'mem', value => { base => 'rsp', disp => 0 }, type => $ptr );
        $mbb->add_instruction(
        ...
        # gate_id was in rsi; use it to compute &gate_table[gate_id]
        my $gate_id = Brocken::Jenny::MIR::MachineOperand->new( kind => 'virt_reg', value => '%gate_id', type => $i64 );
        $mbb->add_instruction(
            Brocken::Jenny::MIR::MachineInstruction->new(
                opcode   => 'mov',
                operands => [ $gate_id, Brocken::Jenny::MIR::MachineOperand->new( kind => 'phys_reg', value => 'rsi' ) ],
                comment  => 'gate_id = rsi'
            )
        );
        ...
        # Add rsp, 8 to deallocate stack slot
        $mbb->add_instruction(
        ...
        $mbb->add_instruction(
            Brocken::Jenny::MIR::MachineInstruction->new(
                opcode   => 'mov',
                operands => [ $rdi, $icb ],
                comment  => 'rdi = ICB (first arg for called function)'
            )
        );

        # rsi = gate_id for called function (second arg)
        my $rsi = Brocken::Jenny::MIR::MachineOperand->new( kind => 'phys_reg', value => 'rsi' );
        $mbb->add_instruction(
        ...
        for my $mbb ( $mf->blocks->@* ) {
            for my $inst ( $mbb->instructions->@* ) {
                ...
                # SSE float opcodes
                    ...
                    my $dst_r      = $resolve->($dst);
                    my $did        = $reg_id->($dst_r);
                    my $label_name = $src->value;
                    my $rex        = 0x48 | ( $did >= 8 ? 4 : 0 );
                    my $modrm      = 0x05 | ( ( $did & 7 ) << 3 );
                    push @func_fixups, { offset => $current_offset->() + 3, type => 'lea_rodata_rel32', target => $label_name };
                    $bytes .= pack( 'C', $rex ) . pack( 'CC', 0x8D, $modrm ) . "\x00\x00\x00\x00";
                }
