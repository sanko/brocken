    ...
    method scan( $source, $filename = 'input' ) {
        ...
        while ( length $remaining > 0 ) {
            ...
            if ( $remaining =~ /^(\s+|#.*(?:\n|$))/u ) {
                ...
                # If we hit a newline, reset line/col
                if ( $match =~ /\n/ ) {
                    $line++;
                    $col = 1;
                }
            ...
            if ( $remaining =~ /^<<([a-zA-Z_]\w*)/u ) {
                ...
                # If there's a semicolon after the marker, consume it now as a separate token
                if ( $remaining =~ /^\s*;/ ) {
                    $remaining =~ s/^\s*;//;
                    push @tokens, Brocken::Token->new( type => 'SEMICOLON', value => ';', line => $line, column => $col, file => $filename );
                }
            ...
            foreach my $type (
                qw(KEYWORD OPERATOR FILE_TEST_OP VARIABLE IDENTIFIER STRING NUMBER SEMICOLON COMMA POD6_CMD ATTRIBUTE HEREDOC_INIT LBRACE RBRACE LBRACKET RBRACKET LPAREN RPAREN)
            ) {
