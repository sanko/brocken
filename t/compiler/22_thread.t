...
subtest 'spawn_thread compile and run' => sub {
    ...
    if (-e $exe) { diag "FILE EXISTS: $exe"; } else { diag "FILE DOES NOT EXIST: $exe"; }
...
subtest 'spawn_thread concurrent execution' => sub {
    ...
    my $source = 'say "before"; my $t = spawn_thread sub { say "child_start"; sleep(2); say "child_end"; }; sleep (1); say "parent_mid"; sleep(1); join_thread($t); say "after";';
    my $lexer  = Brocken::Lexer->new( source => $source );
    my $parser = Brocken::Parser->new( lexer => $lexer );
    my $ast    = $parser->parse();
    my $cfg    = $lowerer->lower($ast);
