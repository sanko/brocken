        ...
        method _encode( $mf, $assignment, $used_callee ) {
            ...
            my $mem_modrm = sub ($mem_op, $reg_idx) {
                ...
                if ( defined $addr->{index} ) {
                    my $index_r = $resolve->(
                        Brocken::Jenny::MIR::MachineOperand->new( kind => 'virt_reg', value => $addr->{index} )
                    );
            ...
            for my $mbb ( $mf->blocks->@* ) {
                for my $inst ( $mbb->instructions->@* ) {
                    ...
                    elsif ( $opcode eq 'add' || $opcode eq 'sub' || $opcode eq 'and' || $opcode eq 'or' || $opcode eq 'xor' || $opcode eq 'adc' || $opcode eq 'sbb' ) {
                        my $dst_r = $resolve->($dst);
                        my $did   = $reg_id->($dst_r);
                        my $bits  = $dst->type ? $dst->type->bits : 64;
        ...
        method _encode( $mf, $assignment, $used_callee ) {
            ...
            for my $mbb ( $mf->blocks->@* ) {
                for my $inst ( $mbb->instructions->@* ) {
                    ...
                    if ( $opcode eq 'label' ) {
                    ...
                    elsif ( $opcode eq 'add' || $opcode eq 'sub' || $opcode eq 'and' || $opcode eq 'or' || $opcode eq 'xor' || $opcode eq 'mul' || $opcode eq 'mulhu' || $opcode eq 'div' || $opcode eq 'divu' || $opcode eq 'slt' || $opcode eq 'sltu' || $opcode eq 'sltiu' ) {
                        my $dst_r = $resolve->($dst);
                        my $did   = $reg_id->($dst_r);
                        my %imm_f3 = ( add => 0, sub => 0, and => 7, or => 6, xor => 4, slt => 2, sltu => 3, sltiu => 3 );
                        my %reg_f7 = ( add => 0x00, sub => 0x20, mul => 0x01, mulhu => 0x01, div => 0x01, divu => 0x01, and => 0x00, or => 0x00, xor => 0x00, slt => 0x00, sltu => 0x00 );
                        my %reg_f3 = ( add => 0, sub => 0, mul => 0, mulhu => 3, div => 0, divu => 1, and => 7, or => 6, xor => 4, slt => 2, sltu => 3, sltiu => 3 );
                        ...
                        if ( defined $addr->{index} ) {
                            ...
                            my $iid = $reg_id->($index_r);
                            my %used; @used{ values %$assignment } = ();
                            my $tmp_r; for my $r ( $platform->registers('caller')->@* ) { $tmp_r = $r, last unless exists $used{$r} }
                        ...
                        if ( defined $addr->{index} ) {
                            ...
                            my $iid = $reg_id->($index_r);
                            my $tmp2 = $tid;
                            my %used2; @used2{ values %$assignment } = (); $used2{$tmp_r} = 1;
        ...
        method _encode( $mf, $assignment, $used_callee ) {
            ...
            for my $mbb ( $mf->blocks->@* ) {
                for my $inst ( $mbb->instructions->@* ) {
                    ...
                    elsif ( $opcode eq 'add' || $opcode eq 'sub' || $opcode eq 'and' || $opcode eq 'or' || $opcode eq 'xor' || $opcode eq 'mul' || $opcode eq 'umulh' || $opcode eq 'udiv' || $opcode eq 'adc' || $opcode eq 'sbb' ) {
                        my $dst_r = $resolve->($dst);
                        my $did   = $reg_id->($dst_r);
     ...
     class Brocken::Jenny::Lowerer::X86_64 {
        method lower($ir_func) {
            ...
            for my $block ( $ir_func->blocks->@* ) {
                ...
                for my $inst ( $block->instructions->@* ) {
                    my $opcode = $inst->opcode;
                    if ( $opcode eq 'add' || $opcode eq 'sub' || $opcode eq 'mul' || $opcode eq 'div' || $opcode eq 'rem' || $opcode eq 'and' || $opcode eq 'or' || $opcode eq 'xor' || $opcode eq 'shl' || $opcode eq 'lshr' || $opcode eq 'ashr' || $opcode eq 'min' || $opcode eq 'max' ) {
                        my ( $lhs, $rhs ) = $inst->operands->@*;
                        my $is_i128 = $inst->type && $inst->type->kind eq 'int' && $inst->type->bits == 128;
                        if ($is_i128 && ($opcode eq 'add' || $opcode eq 'sub' || $opcode eq 'and' || $opcode eq 'or' || $opcode eq 'xor' || $opcode eq 'shl' || $opcode eq 'lshr' || $opcode eq 'ashr' || $opcode eq 'mul' || $opcode eq 'div' || $opcode eq 'rem')) {
                            if ($opcode eq 'shl' || $opcode eq 'lshr' || $opcode eq 'ashr') {
                                ...
                                my $lo_tmp = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . '_t', type => Brocken::Lindsay::IR::Type::i64());
                                if ($rhs->isa('Brocken::Lindsay::IR::Constant')) {
                                    my $amt = $rhs->value;
                                    if ($amt == 0) {
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$lo_dst, $lo_lhs], comment => 'i128 shl/lo by 0'));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$hi_dst, $hi_lhs], comment => 'i128 shl/hi by 0'));
                                    }
                                    elsif ($opcode eq 'shl') {
                                        if ($amt < 64) {
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$lo_dst, $lo_lhs], comment => 'i128 shl lo'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'shl', operands => [$lo_dst, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => $amt, type => Brocken::Lindsay::IR::Type::i64())], comment => 'i128 shl lo by ' . $amt));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$lo_tmp, $lo_lhs], comment => 'i128 shl carry tmp'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'lshr', operands => [$lo_tmp, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 64 - $amt, type => Brocken::Lindsay::IR::Type::i64())], comment => 'i128 shl carry >> ' . (64 - $amt)));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$hi_dst, $hi_lhs], comment => 'i128 shl hi'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'shl', operands => [$hi_dst, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => $amt, type => Brocken::Lindsay::IR::Type::i64())], comment => 'i128 shl hi by ' . $amt));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'or', operands => [$hi_dst, $lo_tmp], comment => 'i128 shl hi |= carry'));
                                        }
                                        elsif ($amt == 64) {
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$hi_dst, $lo_lhs], comment => 'i128 shl hi = lo'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'xor', operands => [$lo_dst, $lo_dst], comment => 'i128 shl lo = 0'));
                                        }
                                        elsif ($amt < 128) {
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$hi_dst, $lo_lhs], comment => 'i128 shl hi = lo'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'shl', operands => [$hi_dst, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => $amt - 64, type => Brocken::Lindsay::IR::Type::i64())], comment => 'i128 shl hi << ' . ($amt - 64)));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'xor', operands => [$lo_dst, $lo_dst], comment => 'i128 shl lo = 0'));
                                        }
                                        else {
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'xor', operands => [$lo_dst, $lo_dst], comment => 'i128 shl lo = 0'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'xor', operands => [$hi_dst, $hi_dst], comment => 'i128 shl hi = 0'));
                                        }
                                    }
                                    elsif ($opcode eq 'lshr') {
                                        if ($amt < 64) {
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$hi_dst, $hi_lhs], comment => 'i128 lshr hi'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'lshr', operands => [$hi_dst, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => $amt, type => Brocken::Lindsay::IR::Type::i64())], comment => 'i128 lshr hi >> ' . $amt));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$lo_tmp, $hi_lhs], comment => 'i128 lshr carry tmp'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'shl', operands => [$lo_tmp, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 64 - $amt, type => Brocken::Lindsay::IR::Type::i64())], comment => 'i128 lshr carry << ' . (64 - $amt)));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$lo_dst, $lo_lhs], comment => 'i128 lshr lo'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'lshr', operands => [$lo_dst, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => $amt, type => Brocken::Lindsay::IR::Type::i64())], comment => 'i128 lshr lo >> ' . $amt));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'or', operands => [$lo_dst, $lo_tmp], comment => 'i128 lshr lo |= carry'));
                                        }
                                        elsif ($amt == 64) {
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$lo_dst, $hi_lhs], comment => 'i128 lshr lo = hi'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'xor', operands => [$hi_dst, $hi_dst], comment => 'i128 lshr hi = 0'));
                                        }
                                        elsif ($amt < 128) {
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$lo_dst, $hi_lhs], comment => 'i128 lshr lo = hi'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'lshr', operands => [$lo_dst, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => $amt - 64, type => Brocken::Lindsay::IR::Type::i64())], comment => 'i128 lshr lo >> ' . ($amt - 64)));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'xor', operands => [$hi_dst, $hi_dst], comment => 'i128 lshr hi = 0'));
                                        }
                                        else {
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'xor', operands => [$lo_dst, $lo_dst], comment => 'i128 lshr lo = 0'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'xor', operands => [$hi_dst, $hi_dst], comment => 'i128 lshr hi = 0'));
                                        }
                                    }
                                    else {
                                        if ($amt < 64) {
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$hi_dst, $hi_lhs], comment => 'i128 ashr hi'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'ashr', operands => [$hi_dst, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => $amt, type => Brocken::Lindsay::IR::Type::i64())], comment => 'i128 ashr hi >> ' . $amt));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$lo_tmp, $hi_lhs], comment => 'i128 ashr carry tmp'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'shl', operands => [$lo_tmp, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 64 - $amt, type => Brocken::Lindsay::IR::Type::i64())], comment => 'i128 ashr carry << ' . (64 - $amt)));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$lo_dst, $lo_lhs], comment => 'i128 ashr lo'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'lshr', operands => [$lo_dst, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => $amt, type => Brocken::Lindsay::IR::Type::i64())], comment => 'i128 ashr lo >> ' . $amt));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'or', operands => [$lo_dst, $lo_tmp], comment => 'i128 ashr lo |= carry'));
                                        }
                                        elsif ($amt == 64) {
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$lo_dst, $hi_lhs], comment => 'i128 ashr lo = hi'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$hi_dst, $hi_lhs], comment => 'i128 ashr hi for sign'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'ashr', operands => [$hi_dst, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 63, type => Brocken::Lindsay::IR::Type::i64())], comment => 'i128 ashr sign extend'));
                                        }
                                        elsif ($amt < 128) {
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$lo_dst, $hi_lhs], comment => 'i128 ashr lo = hi'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'ashr', operands => [$lo_dst, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => $amt - 64, type => Brocken::Lindsay::IR::Type::i64())], comment => 'i128 ashr lo >> ' . ($amt - 64)));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$hi_dst, $hi_lhs], comment => 'i128 ashr hi for sign'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'ashr', operands => [$hi_dst, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 63, type => Brocken::Lindsay::IR::Type::i64())], comment => 'i128 ashr sign extend'));
                                        }
                                        else {
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$lo_dst, $hi_lhs], comment => 'i128 ashr lo = sign'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'ashr', operands => [$lo_dst, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 63, type => Brocken::Lindsay::IR::Type::i64())], comment => 'i128 ashr lo sign'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$hi_dst, $lo_dst], comment => 'i128 ashr hi = sign'));
                                        }
                                    }
                                }
                                else {
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$lo_dst, $lo_lhs], comment => 'i128 var-shl load lo'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$hi_dst, $hi_lhs], comment => 'i128 var-shl load hi'));
                                }
                            }
                            elsif ($opcode eq 'mul') {
                                my ($lo_lhs, $hi_lhs) = $self->_split_i128($lhs);
                                ...
                                my $lo_dst = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . '_lo', type => Brocken::Lindsay::IR::Type::i64());
                                my $hi_dst = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . '_hi', type => Brocken::Lindsay::IR::Type::i64());
                                my $tmp1 = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . '_t1', type => Brocken::Lindsay::IR::Type::i64());
                                my $tmp2 = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . '_t2', type => Brocken::Lindsay::IR::Type::i64());
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$lo_dst, $lo_lhs], comment => 'i128 mul lo'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mul', operands => [$lo_dst, $lo_rhs], comment => 'i128 mul lo = a_lo * b_lo'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$hi_dst, $lo_lhs], comment => 'i128 mul hi'));
                                if ($lo_rhs->kind eq 'imm') {
                                    ...
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$r, $lo_rhs], comment => 'i128 mul rhs lo'));
                                    $lo_rhs = $r;
                                }
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'umulh', operands => [$hi_dst, $lo_rhs], comment => 'i128 mul hi = umulh(a_lo, b_lo)'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$tmp1, $lo_lhs], comment => 'i128 mul tmp1'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mul', operands => [$tmp1, $hi_rhs], comment => 'i128 mul tmp1 = a_lo * b_hi'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'add', operands => [$hi_dst, $tmp1], comment => 'i128 mul hi += tmp1'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$tmp2, $hi_lhs], comment => 'i128 mul tmp2'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mul', operands => [$tmp2, $lo_rhs], comment => 'i128 mul tmp2 = a_hi * b_lo'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'add', operands => [$hi_dst, $tmp2], comment => 'i128 mul hi += tmp2'));
                            }
                            elsif ($opcode eq 'div' || $opcode eq 'rem') {
                                my ($lo_lhs, $hi_lhs) = $self->_split_i128($lhs);
                                my ($lo_rhs, $hi_rhs) = $self->_split_i128($rhs);
                                my ($lo_dst, $hi_dst) = $self->_split_i128($inst);
                                my $q_lo = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . '_q_lo', type => Brocken::Lindsay::IR::Type::i64());
                                my $q_hi = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . '_q_hi', type => Brocken::Lindsay::IR::Type::i64());
                                my $r_lo = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . '_r_lo', type => Brocken::Lindsay::IR::Type::i64());
                                my $r_hi = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . '_r_hi', type => Brocken::Lindsay::IR::Type::i64());
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$q_lo, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0)], comment => 'i128 div q_lo=0'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$q_hi, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0)], comment => 'i128 div q_hi=0'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$r_lo, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0)], comment => 'i128 div r_lo=0'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$r_hi, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0)], comment => 'i128 div r_hi=0'));
                                for my $ii (reverse 0 .. 127) {
                                    my $val = $ii >= 64 ? $hi_lhs : $lo_lhs;
                                    my $shift = $ii >= 64 ? $ii - 64 : $ii;
                                    my $bit = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . "_b$ii", type => Brocken::Lindsay::IR::Type::i64());
                                    my $carry = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . "_c$ii", type => Brocken::Lindsay::IR::Type::i64());
                                    my $t_hi_gt = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . "_hgt$ii", type => Brocken::Lindsay::IR::Type::i64());
                                    my $t_hi_eq = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . "_heq$ii", type => Brocken::Lindsay::IR::Type::i64());
                                    my $t_lo_ge = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . "_lge$ii", type => Brocken::Lindsay::IR::Type::i64());
                                    my $t_cond = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . "_cond$ii", type => Brocken::Lindsay::IR::Type::i64());
                                    my $t_neg = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . "_neg$ii", type => Brocken::Lindsay::IR::Type::i64());
                                    my $t_mdl = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . "_mdl$ii", type => Brocken::Lindsay::IR::Type::i64());
                                    my $t_bor = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . "_bor$ii", type => Brocken::Lindsay::IR::Type::i64());
                                    my $t_mdh = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . "_mdh$ii", type => Brocken::Lindsay::IR::Type::i64());
                                        # bit = (val >> shift) & 1
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$bit, $val], comment => "i128 div bit$ii val"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'lshr', operands => [$bit, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => $shift)], comment => "i128 div bit$ii shr"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'and', operands => [$bit, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 1)], comment => "i128 div bit$ii and"));
                                        # carry = r_lo >> 63
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$carry, $r_lo], comment => "i128 div carry$ii load"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'lshr', operands => [$carry, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 63)], comment => "i128 div carry$ii shr"));
                                        # r_lo = (r_lo << 1) | bit
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$r_lo, $r_lo], comment => "i128 div r_lo$ii shl"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'shl', operands => [$r_lo, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 1)], comment => "i128 div r_lo$ii shl"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'or', operands => [$r_lo, $bit], comment => "i128 div r_lo$ii or"));
                                        # r_hi = (r_hi << 1) | carry
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$r_hi, $r_hi], comment => "i128 div r_hi$ii shl"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'shl', operands => [$r_hi, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 1)], comment => "i128 div r_hi$ii shl"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'or', operands => [$r_hi, $carry], comment => "i128 div r_hi$ii or"));
                                        # cond_hi_gt = r_hi > d_hi  (unsigned greater than)
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$t_hi_gt, $r_hi], comment => "i128 div hgt$ii load"));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'cmp', operands => [$t_hi_gt, $hi_rhs], comment => "i128 div hgt$ii cmp"));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$t_hi_gt, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0)], comment => "i128 div hgt$ii zero"));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'seta', operands => [$t_hi_gt], comment => "i128 div hgt$ii seta"));
                                    # cond_hi_eq = r_hi == d_hi
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$t_hi_eq, $r_hi], comment => "i128 div heq$ii load"));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'cmp', operands => [$t_hi_eq, $hi_rhs], comment => "i128 div heq$ii cmp"));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$t_hi_eq, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0)], comment => "i128 div heq$ii zero"));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'sete', operands => [$t_hi_eq], comment => "i128 div heq$ii sete"));
                                    # cond_lo_ge = !(r_lo < d_lo)  i.e., r_lo >= d_lo  (unsigned)
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$t_lo_ge, $r_lo], comment => "i128 div lge$ii load"));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'cmp', operands => [$t_lo_ge, $lo_rhs], comment => "i128 div lge$ii cmp"));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$t_lo_ge, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0)], comment => "i128 div lge$ii zero"));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'setae', operands => [$t_lo_ge], comment => "i128 div lge$ii setae"));
                                    # cond = cond_hi_gt | (cond_hi_eq & cond_lo_ge)
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'and', operands => [$t_hi_eq, $t_lo_ge], comment => "i128 div cond$ii and"));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'or', operands => [$t_hi_gt, $t_hi_eq], comment => "i128 div cond$ii or"));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$t_cond, $t_hi_gt], comment => "i128 div cond$ii save"));
                                    # neg_cond = -cond  (0 -> 0, 1 -> -1 = all-ones mask)
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$t_neg, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0)], comment => "i128 div neg$ii zero"));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'sub', operands => [$t_neg, $t_cond], comment => "i128 div neg$ii sub"));
                                    # masked_d_lo = d_lo & neg_cond
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$t_mdl, $lo_rhs], comment => "i128 div mdl$ii load"));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'and', operands => [$t_mdl, $t_neg], comment => "i128 div mdl$ii and"));
                                    # borrow = r_lo < masked_d_lo  (unsigned)
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$t_bor, $r_lo], comment => "i128 div bor$ii load"));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'cmp', operands => [$t_bor, $t_mdl], comment => "i128 div bor$ii cmp"));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$t_bor, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0)], comment => "i128 div bor$ii zero"));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'setb', operands => [$t_bor], comment => "i128 div bor$ii setb"));
                                    # r_lo -= masked_d_lo
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$r_lo, $r_lo], comment => "i128 div r_lo$ii sub"));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'sub', operands => [$r_lo, $t_mdl], comment => "i128 div r_lo$ii sub"));
                                    # masked_d_hi = d_hi & neg_cond
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$t_mdh, $hi_rhs], comment => "i128 div mdh$ii load"));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'and', operands => [$t_mdh, $t_neg], comment => "i128 div mdh$ii and"));
                                    # r_hi -= masked_d_hi + borrow
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$r_hi, $r_hi], comment => "i128 div r_hi$ii sub"));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'sub', operands => [$r_hi, $t_mdh], comment => "i128 div r_hi$ii sub mdh"));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'sub', operands => [$r_hi, $t_bor], comment => "i128 div r_hi$ii sub bor"));
                                    # Q_bit = (1 << (ii % 64)) & neg_cond
                                    my $qbit_val = 1 << ($ii % 64);
                                    my $qbit = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . "_qb$ii", type => Brocken::Lindsay::IR::Type::i64());
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$qbit, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => $qbit_val)], comment => "i128 div qb$ii val"));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'and', operands => [$qbit, $t_neg], comment => "i128 div qb$ii and"));
                                    if ($ii >= 64) {
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$q_hi, $q_hi], comment => "i128 div q_hi$ii or"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'or', operands => [$q_hi, $qbit], comment => "i128 div q_hi$ii or"));
                                    }
                                    else {
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$q_lo, $q_lo], comment => "i128 div q_lo$ii or"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'or', operands => [$q_lo, $qbit], comment => "i128 div q_lo$ii or"));
                                    }
                                }
                                my $out_lo = $opcode eq 'div' ? $q_lo : $r_lo;
                                my $out_hi = $opcode eq 'div' ? $q_hi : $r_hi;
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$lo_dst, $out_lo], comment => 'i128 div store lo'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$hi_dst, $out_hi], comment => 'i128 div store hi'));
                            }
                            else {
                                my ($lo_lhs, $hi_lhs) = $self->_split_i128($lhs);
                                my ($lo_rhs, $hi_rhs) = $self->_split_i128($rhs);
                                my $lo_dst = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . '_lo', type => Brocken::Lindsay::IR::Type::i64());
                                my $hi_dst = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . '_hi', type => Brocken::Lindsay::IR::Type::i64());
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$lo_dst, $lo_lhs], comment => 'i128 load lo ' . ($lhs->name || $lhs->value)));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => $opcode, operands => [$lo_dst, $lo_rhs], comment => 'i128 ' . $opcode . ' lo'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$hi_dst, $hi_lhs], comment => 'i128 load hi ' . ($lhs->name || $lhs->value)));
                                if ($opcode eq 'add') {
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'adc', operands => [$hi_dst, $hi_rhs], comment => 'i128 adc hi'));
                                }
                                elsif ($opcode eq 'sub') {
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'sbb', operands => [$hi_dst, $hi_rhs], comment => 'i128 sbb hi'));
                                }
                                else {
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => $opcode, operands => [$hi_dst, $hi_rhs], comment => 'i128 ' . $opcode . ' hi'));
                                }
                            }
                        }
                        else {
                        ...
                        if ($lhs->type && $lhs->type->kind eq 'float') {
                            ...
                            if ($pred eq 'eq' || $pred eq 'ne') {
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$t0, $lo_lhs], comment => 'i128 icmp lo'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'xor', operands => [$t0, $lo_rhs], comment => 'i128 icmp lo_diff'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$t1, $hi_lhs], comment => 'i128 icmp hi'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'xor', operands => [$t1, $hi_rhs], comment => 'i128 icmp hi_diff'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'or', operands => [$t0, $t1], comment => 'i128 icmp combined'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$dst, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0)], comment => 'i128 icmp zero dst'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'cmp', operands => [$t0, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0)], comment => 'i128 icmp test'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => ($pred eq 'eq' ? 'sete' : 'setne'), operands => [$dst], comment => 'i128 icmp ' . $pred));
                            }
                            elsif ($pred eq 'ult' || $pred eq 'slt') {
                                my $set_op = $pred eq 'ult' ? 'setb' : 'setl';
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$t0, $hi_lhs], comment => 'i128 icmp hi'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'cmp', operands => [$t0, $hi_rhs], comment => 'i128 icmp hi_cmp'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$t0, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0)], comment => 'i128 icmp zero'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => $set_op, operands => [$t0], comment => 'i128 icmp hi_' . $pred));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$t1, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0)], comment => 'i128 icmp zero'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'sete', operands => [$t1], comment => 'i128 icmp hi_eq'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$t2, $lo_lhs], comment => 'i128 icmp lo'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'cmp', operands => [$t2, $lo_rhs], comment => 'i128 icmp lo_cmp'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$t2, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0)], comment => 'i128 icmp zero'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'setb', operands => [$t2], comment => 'i128 icmp lo_lt'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'and', operands => [$t1, $t2], comment => 'i128 icmp hi_eq&lo_lt'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'or', operands => [$t0, $t1], comment => 'i128 icmp result'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$dst, $t0], comment => 'i128 icmp ' . $pred));
                            }
                            elsif ($pred eq 'ugt' || $pred eq 'sgt') {
                                my $set_op = $pred eq 'ugt' ? 'setb' : 'setl';
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$t0, $hi_rhs], comment => 'i128 icmp hi (swapped)'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'cmp', operands => [$t0, $hi_lhs], comment => 'i128 icmp hi_cmp'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$t0, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0)], comment => 'i128 icmp zero'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => $set_op, operands => [$t0], comment => 'i128 icmp hi_' . $pred));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$t1, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0)], comment => 'i128 icmp zero'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'sete', operands => [$t1], comment => 'i128 icmp hi_eq'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$t2, $lo_rhs], comment => 'i128 icmp lo (swapped)'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'cmp', operands => [$t2, $lo_lhs], comment => 'i128 icmp lo_cmp'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$t2, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0)], comment => 'i128 icmp zero'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'setb', operands => [$t2], comment => 'i128 icmp lo_lt'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'and', operands => [$t1, $t2], comment => 'i128 icmp hi_eq&lo_lt'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'or', operands => [$t0, $t1], comment => 'i128 icmp result'));
                                ...
                                my ($set_lt, $swap) = $pred eq 'ule' ? ('setb', 1) : $pred eq 'uge' ? ('setb', 0) : $pred eq 'sle' ? ('setl', 1) : ('setl', 0);
                                ...
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$t0, $hi_a], comment => 'i128 icmp hi'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'cmp', operands => [$t0, $hi_b], comment => 'i128 icmp hi_cmp'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$t0, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0)], comment => 'i128 icmp zero'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => $set_lt, operands => [$t0], comment => 'i128 icmp hi_lt'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$t1, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0)], comment => 'i128 icmp zero'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'sete', operands => [$t1], comment => 'i128 icmp hi_eq'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$t2, $lo_a], comment => 'i128 icmp lo'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'cmp', operands => [$t2, $lo_b], comment => 'i128 icmp lo_cmp'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$t2, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0)], comment => 'i128 icmp zero'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'setb', operands => [$t2], comment => 'i128 icmp lo_lt'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'and', operands => [$t1, $t2], comment => 'i128 icmp hi_eq&lo_lt'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'or', operands => [$t0, $t1], comment => 'i128 icmp result'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$dst, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0)], comment => 'i128 icmp zero dst'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'cmp', operands => [$t0, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0)], comment => 'i128 icmp not test'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'sete', operands => [$dst], comment => 'i128 icmp ' . $pred));
                            }
                        }
                        else {
                            $mbb->add_instruction(
                        ...
                        if ($is_i128) {
                            my $lo_dst = Brocken::Jenny::MIR::MachineOperand->new(
                                kind => 'virt_reg', value => $inst->name . '_lo', type => Brocken::Lindsay::IR::Type::i64()
                            );
                            my $hi_dst = Brocken::Jenny::MIR::MachineOperand->new(
                                kind => 'virt_reg', value => $inst->name . '_hi', type => Brocken::Lindsay::IR::Type::i64()
                            );
                            ...
                            $mbb->add_instruction(
                                Brocken::Jenny::MIR::MachineInstruction->new(
                                    opcode => 'load', operands => [ $lo_dst, $mem_lo ],
                                    comment => 'i128 load lo'
                                )
                            );
                            $mbb->add_instruction(
                                Brocken::Jenny::MIR::MachineInstruction->new(
                                    opcode => 'load', operands => [ $hi_dst, $mem_hi ],
                        ...
                        if ( $idx->isa('Brocken::Lindsay::IR::Constant') ) {
                            ...
                            $mbb->add_instruction(
                                Brocken::Jenny::MIR::MachineInstruction->new(
                                    opcode => 'lea', operands => [ $dst, Brocken::Jenny::MIR::MachineOperand->new( kind => 'mem', value => { base => $ptr->name, disp => $offset }, type => $ptr->type ) ],
                            ...
                            $mbb->add_instruction(
                                Brocken::Jenny::MIR::MachineInstruction->new(
                                    opcode => 'lea', operands => [ $dst, Brocken::Jenny::MIR::MachineOperand->new( kind => 'mem', value => { base => $ptr->name, index => $idx->name, scale => $scale, disp => 0 }, type => $ptr->type ) ],
                ...
                for my $inst ( $block->instructions->@* ) {
                    ...
                    if ( $opcode eq 'add' || $opcode eq 'sub' || $opcode eq 'mul' || $opcode eq 'div' || $opcode eq 'rem' || $opcode eq 'and' || $opcode eq 'or' || $opcode eq 'xor' || $opcode eq 'shl' || $opcode eq 'lshr' || $opcode eq 'ashr' || $opcode eq 'min' || $opcode eq 'max' ) {
                        ...
                        if ($is_float) {
                            ...
                            if ($is_i128 && ($opcode eq 'add' || $opcode eq 'sub' || $opcode eq 'and' || $opcode eq 'or' || $opcode eq 'xor' || $opcode eq 'shl' || $opcode eq 'lshr' || $opcode eq 'ashr' || $opcode eq 'mul' || $opcode eq 'div' || $opcode eq 'rem')) {
                                ...
                                my $lo_tmp = Brocken::Jenny::MIR::MachineOperand->new(
                                    kind => 'virt_reg', value => $inst->name . '_t', type => Brocken::Lindsay::IR::Type::i64()
                                );
                                if ($opcode eq 'shl' || $opcode eq 'lshr' || $opcode eq 'ashr') {
                                    if ($rhs->isa('Brocken::Lindsay::IR::Constant')) {
                                        my $amt = $rhs->value;
                                        if ($amt == 0) {
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$lo_dst, $lo_lhs], comment => 'i128 shl lo by 0'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$hi_dst, $hi_lhs], comment => 'i128 shl hi by 0'));
                                        }
                                        elsif ($opcode eq 'shl') {
                                            if ($amt < 64) {
                                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$lo_dst, $lo_lhs], comment => 'i128 shl lo'));
                                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'shl', operands => [$lo_dst, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => $amt, type => Brocken::Lindsay::IR::Type::i64())], comment => 'i128 shl lo<<' . $amt));
                                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$lo_tmp, $lo_lhs], comment => 'i128 shl carry'));
                                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'lshr', operands => [$lo_tmp, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 64 - $amt, type => Brocken::Lindsay::IR::Type::i64())], comment => 'i128 shl carry>>' . (64 - $amt)));
                                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$hi_dst, $hi_lhs], comment => 'i128 shl hi'));
                                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'shl', operands => [$hi_dst, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => $amt, type => Brocken::Lindsay::IR::Type::i64())], comment => 'i128 shl hi<<' . $amt));
                                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'or', operands => [$hi_dst, $lo_tmp], comment => 'i128 shl hi|=carry'));
                                            }
                                            elsif ($amt == 64) {
                                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$hi_dst, $lo_lhs], comment => 'i128 shl hi=lo'));
                                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'xor', operands => [$lo_dst, $lo_dst], comment => 'i128 shl lo=0'));
                                            }
                                            elsif ($amt < 128) {
                                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$hi_dst, $lo_lhs], comment => 'i128 shl hi=lo'));
                                            ...
                                            if ($amt < 64) {
                                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$hi_dst, $hi_lhs], comment => 'i128 lshr hi'));
                                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'lshr', operands => [$hi_dst, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => $amt, type => Brocken::Lindsay::IR::Type::i64())], comment => 'i128 lshr hi>>' . $amt));
                                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$lo_tmp, $hi_lhs], comment => 'i128 lshr carry'));
                                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'shl', operands => [$lo_tmp, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 64 - $amt, type => Brocken::Lindsay::IR::Type::i64())], comment => 'i128 lshr carry<<' . (64 - $amt)));
                                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$lo_dst, $lo_lhs], comment => 'i128 lshr lo'));
                                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'lshr', operands => [$lo_dst, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => $amt, type => Brocken::Lindsay::IR::Type::i64())], comment => 'i128 lshr lo>>' . $amt));
                                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'or', operands => [$lo_dst, $lo_tmp], comment => 'i128 lshr lo|=carry'));
                                            }
                                            elsif ($amt == 64) {
                                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$lo_dst, $hi_lhs], comment => 'i128 lshr lo=hi'));
                                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'xor', operands => [$hi_dst, $hi_dst], comment => 'i128 lshr hi=0'));
                                            }
                                            elsif ($amt < 128) {
                                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$lo_dst, $hi_lhs], comment => 'i128 lshr lo=hi'));
                                            ...
                                            if ($amt < 64) {
                                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$hi_dst, $hi_lhs], comment => 'i128 ashr hi'));
                                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'ashr', operands => [$hi_dst, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => $amt, type => Brocken::Lindsay::IR::Type::i64())], comment => 'i128 ashr hi>>' . $amt));
                                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$lo_tmp, $hi_lhs], comment => 'i128 ashr carry'));
                                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'shl', operands => [$lo_tmp, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 64 - $amt, type => Brocken::Lindsay::IR::Type::i64())], comment => 'i128 ashr carry<<' . (64 - $amt)));
                                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$lo_dst, $lo_lhs], comment => 'i128 ashr lo'));
                                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'lshr', operands => [$lo_dst, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => $amt, type => Brocken::Lindsay::IR::Type::i64())], comment => 'i128 ashr lo>>' . $amt));
                                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'or', operands => [$lo_dst, $lo_tmp], comment => 'i128 ashr lo|=carry'));
                                            }
                                            elsif ($amt == 64) {
                                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$lo_dst, $hi_lhs], comment => 'i128 ashr lo=hi'));
                                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$hi_dst, $hi_lhs], comment => 'i128 ashr hi sign'));
                                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'ashr', operands => [$hi_dst, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 63, type => Brocken::Lindsay::IR::Type::i64())], comment => 'i128 ashr sign extend'));
                                            }
                                            elsif ($amt < 128) {
                                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$lo_dst, $hi_lhs], comment => 'i128 ashr lo=hi'));
                                                ...
                                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$lo_dst, $hi_lhs], comment => 'i128 ashr lo sign'));
                                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'ashr', operands => [$lo_dst, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 63, type => Brocken::Lindsay::IR::Type::i64())], comment => 'i128 ashr lo>>63'));
                                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$hi_dst, $lo_dst], comment => 'i128 ashr hi=sign'));
                                            }
                                        }
                                    }
                                    else {
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$lo_dst, $lo_lhs], comment => 'i128 var-shl lo'));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$hi_dst, $hi_lhs], comment => 'i128 var-shl hi'));
                                    }
                                }
                                elsif ($opcode eq 'mul') {
                                    my ($lo_rhs, $hi_rhs) = $self->_split_i128($rhs);
                                    my $tmp1 = Brocken::Jenny::MIR::MachineOperand->new(
                                        kind => 'virt_reg', value => $inst->name . '_t1', type => Brocken::Lindsay::IR::Type::i64()
                                    );
                                    my $tmp2 = Brocken::Jenny::MIR::MachineOperand->new(
                                        kind => 'virt_reg', value => $inst->name . '_t2', type => Brocken::Lindsay::IR::Type::i64()
                                    );
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$lo_dst, $lo_lhs], comment => 'i128 mul lo'));
                                    if ($lo_rhs->kind eq 'imm') {
                                        my $r = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . '_rl', type => Brocken::Lindsay::IR::Type::i64());
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$r, $lo_rhs], comment => 'i128 mul rhs lo'));
                                        $lo_rhs = $r;
                                    }
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mul', operands => [$lo_dst, $lo_rhs], comment => 'i128 mul lo = a_lo * b_lo'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$hi_dst, $lo_lhs], comment => 'i128 mul hi'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'umulh', operands => [$hi_dst, $lo_rhs], comment => 'i128 mul hi = umulh(a_lo, b_lo)'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$tmp1, $lo_lhs], comment => 'i128 mul tmp1'));
                                    if ($hi_rhs->kind eq 'imm') {
                                        ...
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$r, $hi_rhs], comment => 'i128 mul rhs hi'));
                                        $hi_rhs = $r;
                                    }
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mul', operands => [$tmp1, $hi_rhs], comment => 'i128 mul tmp1 = a_lo * b_hi'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'add', operands => [$hi_dst, $tmp1], comment => 'i128 mul hi += tmp1'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$tmp2, $hi_lhs], comment => 'i128 mul tmp2'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mul', operands => [$tmp2, $lo_rhs], comment => 'i128 mul tmp2 = a_hi * b_lo'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'add', operands => [$hi_dst, $tmp2], comment => 'i128 mul hi += tmp2'));
                                }
                                elsif ($opcode eq 'add') {
                                    ...
                                    my $carry = Brocken::Jenny::MIR::MachineOperand->new(
                                        kind => 'virt_reg', value => $inst->name . '_c', type => Brocken::Lindsay::IR::Type::i64()
                                    );
                                    my ($lo_rhs, $hi_rhs) = $self->_split_i128($rhs);
                                    $mbb->add_instruction(
                                        Brocken::Jenny::MIR::MachineInstruction->new(
                                    ...
                                    my $r_lo = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . '_r_lo', type => Brocken::Lindsay::IR::Type::i64());
                                    my $r_hi = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . '_r_hi', type => Brocken::Lindsay::IR::Type::i64());
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$q_lo, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0)], comment => 'i128 div q_lo=0'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$q_hi, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0)], comment => 'i128 div q_hi=0'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$r_lo, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0)], comment => 'i128 div r_lo=0'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$r_hi, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0)], comment => 'i128 div r_hi=0'));
                                    for my $ii (reverse 0 .. 127) {
                                        my $val = $ii >= 64 ? $hi_lhs : $lo_lhs;
                                        my $shift = $ii >= 64 ? $ii - 64 : $ii;
                                        my $bit = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . "_b$ii", type => Brocken::Lindsay::IR::Type::i64());
                                        ...
                                        my $t_mdh = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . "_mdh$ii", type => Brocken::Lindsay::IR::Type::i64());
                                        # bit = (val >> shift) & 1
                                        my $one = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . "_one$ii", type => Brocken::Lindsay::IR::Type::i64());
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$bit, $val], comment => "i128 div bit$ii val"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$one, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 1)], comment => "i128 div bit$ii one"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'lshr', operands => [$bit, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => $shift, type => Brocken::Lindsay::IR::Type::i64())], comment => "i128 div bit$ii shr"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'and', operands => [$bit, $one], comment => "i128 div bit$ii and"));
                                        # carry = r_lo >> 63
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$carry, $r_lo], comment => "i128 div carry$ii load"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'lshr', operands => [$carry, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 63)], comment => "i128 div carry$ii shr"));
                                        # r_lo = (r_lo << 1) | bit
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$r_lo, $r_lo], comment => "i128 div r_lo$ii shl"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'shl', operands => [$r_lo, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 1)], comment => "i128 div r_lo$ii shl"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'or', operands => [$r_lo, $bit], comment => "i128 div r_lo$ii or"));
                                        # r_hi = (r_hi << 1) | carry
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$r_hi, $r_hi], comment => "i128 div r_hi$ii shl"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'shl', operands => [$r_hi, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 1)], comment => "i128 div r_hi$ii shl"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'or', operands => [$r_hi, $carry], comment => "i128 div r_hi$ii or"));
                                        # cond_hi_gt = r_hi > d_hi  (unsigned greater than)
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$t_hi_gt, $r_hi], comment => "i128 div hgt$ii load"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'cmp', operands => [$t_hi_gt, $hi_rhs], comment => "i128 div hgt$ii cmp"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'cset_hi', operands => [$t_hi_gt], comment => "i128 div hgt$ii cset_hi"));
                                        # cond_hi_eq = r_hi == d_hi
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$t_hi_eq, $r_hi], comment => "i128 div heq$ii load"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'cmp', operands => [$t_hi_eq, $hi_rhs], comment => "i128 div heq$ii cmp"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'cset_eq', operands => [$t_hi_eq], comment => "i128 div heq$ii cset_eq"));
                                        # cond_lo_ge = r_lo >= d_lo  (unsigned, carry set)
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$t_lo_ge, $r_lo], comment => "i128 div lge$ii load"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'cmp', operands => [$t_lo_ge, $lo_rhs], comment => "i128 div lge$ii cmp"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'cset_cs', operands => [$t_lo_ge], comment => "i128 div lge$ii cset_cs"));
                                        # cond = cond_hi_gt | (cond_hi_eq & cond_lo_ge)
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'and', operands => [$t_hi_eq, $t_lo_ge], comment => "i128 div cond$ii and"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'or', operands => [$t_hi_gt, $t_hi_eq], comment => "i128 div cond$ii or"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$t_cond, $t_hi_gt], comment => "i128 div cond$ii save"));
                                        # neg_cond = -cond
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$t_neg, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0)], comment => "i128 div neg$ii zero"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'sub', operands => [$t_neg, $t_cond], comment => "i128 div neg$ii sub"));
                                        # masked_d_lo = d_lo & neg_cond
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$t_mdl, $lo_rhs], comment => "i128 div mdl$ii load"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'and', operands => [$t_mdl, $t_neg], comment => "i128 div mdl$ii and"));
                                        # borrow = r_lo < masked_d_lo  (unsigned less than)
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$t_bor, $r_lo], comment => "i128 div bor$ii load"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'cmp', operands => [$t_bor, $t_mdl], comment => "i128 div bor$ii cmp"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'cset_cc', operands => [$t_bor], comment => "i128 div bor$ii cset_cc"));
                                        # r_lo -= masked_d_lo
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$r_lo, $r_lo], comment => "i128 div r_lo$ii sub"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'sub', operands => [$r_lo, $t_mdl], comment => "i128 div r_lo$ii sub"));
                                        # masked_d_hi = d_hi & neg_cond
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$t_mdh, $hi_rhs], comment => "i128 div mdh$ii load"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'and', operands => [$t_mdh, $t_neg], comment => "i128 div mdh$ii and"));
                                        # r_hi -= masked_d_hi + borrow
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$r_hi, $r_hi], comment => "i128 div r_hi$ii sub"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'sub', operands => [$r_hi, $t_mdh], comment => "i128 div r_hi$ii sub mdh"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'sub', operands => [$r_hi, $t_bor], comment => "i128 div r_hi$ii sub bor"));
                                        # Q_bit = (1 << (ii % 64)) & neg_cond
                                        my $qbit_val = 1 << ($ii % 64);
                                        my $qbit = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . "_qb$ii", type => Brocken::Lindsay::IR::Type::i64());
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$qbit, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => $qbit_val)], comment => "i128 div qb$ii val"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'and', operands => [$qbit, $t_neg], comment => "i128 div qb$ii and"));
                                        if ($ii >= 64) {
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$q_hi, $q_hi], comment => "i128 div q_hi$ii or"));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'or', operands => [$q_hi, $qbit], comment => "i128 div q_hi$ii or"));
                                        }
                                        else {
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$q_lo, $q_lo], comment => "i128 div q_lo$ii or"));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'or', operands => [$q_lo, $qbit], comment => "i128 div q_lo$ii or"));
                                        }
                                    }
                                    my $out_lo = $opcode eq 'div' ? $q_lo : $r_lo;
                                    my $out_hi = $opcode eq 'div' ? $q_hi : $r_hi;
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$lo_dst, $out_lo], comment => 'i128 div store lo'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$hi_dst, $out_hi], comment => 'i128 div store hi'));
                                }
                                else {
                                    my ($lo_rhs, $hi_rhs) = $self->_split_i128($rhs);
                                    if ($lo_rhs->kind eq 'imm') {
                        ...
                        if ($is_i128) {
                            ...
                            my $mem_lo = Brocken::Jenny::MIR::MachineOperand->new(
                                kind  => 'mem', value => { base => $ptr->name, disp => 0 }, type => Brocken::Lindsay::IR::Type::i64()
                            );
                            my $mem_hi = Brocken::Jenny::MIR::MachineOperand->new(
                                kind  => 'mem', value => { base => $ptr->name, disp => 8 }, type => Brocken::Lindsay::IR::Type::i64()
                            );
                            $mbb->add_instruction(
                                Brocken::Jenny::MIR::MachineInstruction->new(
                                    opcode => 'load', operands => [ $lo_dst, $mem_lo ], comment => 'i128 load lo'
                                )
                            );
                            $mbb->add_instruction(
                                Brocken::Jenny::MIR::MachineInstruction->new(
                                    opcode => 'load', operands => [ $hi_dst, $mem_hi ], comment => 'i128 load hi'
                                )
                            );
                        }
                        else {
                        ...
                        if ($is_i128) {
                            ...
                            $mbb->add_instruction(
                                Brocken::Jenny::MIR::MachineInstruction->new(
                                    opcode   => ($lo->kind eq 'imm' ? 'store_imm' : 'store'),
                                    operands => [ $mem_lo, $lo ], comment => 'i128 store lo'
                                )
                            );
                            $mbb->add_instruction(
                                Brocken::Jenny::MIR::MachineInstruction->new(
                                    opcode   => ($hi->kind eq 'imm' ? 'store_imm' : 'store'),
                                    operands => [ $mem_hi, $hi ], comment => 'i128 store hi'
                                )
                            );
                        }
                        else {
                        ...
                        $mbb->add_instruction(
                            ...
                            $mbb->add_instruction(
                                Brocken::Jenny::MIR::MachineInstruction->new(
                                    opcode => 'mov', operands => [ $dst, Brocken::Jenny::MIR::MachineOperand->new( kind => 'virt_reg', value => $ptr->name ) ],
                                    comment => 'gep: mv ptr'
                                )
                            );
                            if ( $idx->isa('Brocken::Lindsay::IR::Constant') ) {
                                ...
                                $mbb->add_instruction(
                                    Brocken::Jenny::MIR::MachineInstruction->new(
                                        opcode => 'add', operands => [ $dst, Brocken::Jenny::MIR::MachineOperand->new( kind => 'imm', value => $offset ) ],
                                ...
                                $mbb->add_instruction(
                                    Brocken::Jenny::MIR::MachineInstruction->new(
                                        opcode => 'add', operands => [ $dst, Brocken::Jenny::MIR::MachineOperand->new( kind => 'virt_reg', value => $idx->name ) ],
                            ...
                            if ($lhs->type && $lhs->type->kind eq 'float') {
                                ...
                                if ($pred eq 'eq' || $pred eq 'ne') {
                                    if ($lo_rhs->kind eq 'imm') {
                                        my $r = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . '_rl', type => Brocken::Lindsay::IR::Type::i64());
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$r, $lo_rhs], comment => 'i128 icmp rhs lo'));
                                        $lo_rhs = $r;
                                    }
                                    if ($hi_rhs->kind eq 'imm') {
                                        my $r = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . '_rh', type => Brocken::Lindsay::IR::Type::i64());
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$r, $hi_rhs], comment => 'i128 icmp rhs hi'));
                                        $hi_rhs = $r;
                                    }
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$t0, $lo_lhs], comment => 'i128 icmp lo'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'xor', operands => [$t0, $lo_rhs], comment => 'i128 icmp lo_diff'));
                                    ...
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'or', operands => [$t0, $t1], comment => 'i128 icmp combined'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'cmp', operands => [$t0, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0)], comment => 'i128 icmp test'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => ($pred eq 'eq' ? 'cset_eq' : 'cset_ne'), operands => [$dst], comment => 'i128 icmp ' . $pred));
                                }
                                elsif ($pred eq 'ult' || $pred eq 'slt') {
                                    my $cset_hi = $pred eq 'ult' ? 'cset_cc' : 'cset_lt';
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$t0, $hi_lhs], comment => 'i128 icmp hi'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'cmp', operands => [$t0, $hi_rhs], comment => 'i128 icmp hi_cmp'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => $cset_hi, operands => [$t0], comment => 'i128 icmp hi_' . $pred));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'cset_eq', operands => [$t1], comment => 'i128 icmp hi_eq'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$t2, $lo_lhs], comment => 'i128 icmp lo'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'cmp', operands => [$t2, $lo_rhs], comment => 'i128 icmp lo_cmp'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'cset_cc', operands => [$t2], comment => 'i128 icmp lo_lt'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'and', operands => [$t1, $t2], comment => 'i128 icmp hi_eq&lo_lt'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'or', operands => [$t0, $t1], comment => 'i128 icmp result'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$dst, $t0], comment => 'i128 icmp ' . $pred));
                                }
                                    ...
                                    my ($cset_hi, $swap) = $pred eq 'ule' ? ('cset_cc', 1) : $pred eq 'uge' ? ('cset_cc', 0) : $pred eq 'sle' ? ('cset_lt', 1) : ('cset_lt', 0);
                                    my ($hi_a, $hi_b, $lo_a, $lo_b) = $swap ? ($hi_rhs, $hi_lhs, $lo_rhs, $lo_lhs) : ($hi_lhs, $hi_rhs, $lo_lhs, $lo_rhs);
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$t0, $hi_a], comment => 'i128 icmp hi'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'cmp', operands => [$t0, $hi_b], comment => 'i128 icmp hi_cmp'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => $cset_hi, operands => [$t0], comment => 'i128 icmp hi_lt'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'cset_eq', operands => [$t1], comment => 'i128 icmp hi_eq'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$t2, $lo_a], comment => 'i128 icmp lo'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'cmp', operands => [$t2, $lo_b], comment => 'i128 icmp lo_cmp'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'cset_cc', operands => [$t2], comment => 'i128 icmp lo_lt'));
                                    ...
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mov', operands => [$dst, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0)], comment => 'i128 icmp zero dst'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'cmp', operands => [$t0, Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0)], comment => 'i128 icmp not test'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'cset_eq', operands => [$dst], comment => 'i128 icmp ' . $pred));
                                }
                            }
                            else {
                                $mbb->add_instruction(
    ...
    # ---------------------------------------------------------------------------
        ...
        method lower($ir_func) {
            ...
            for my $block ( $ir_func->blocks->@* ) {
                ...
                for my $inst ( $block->instructions->@* ) {
                    my $opcode = $inst->opcode;
                    if ( $opcode eq 'add' || $opcode eq 'sub' || $opcode eq 'mul' || $opcode eq 'div' || $opcode eq 'rem' || $opcode eq 'and' || $opcode eq 'or' || $opcode eq 'xor' || $opcode eq 'shl' || $opcode eq 'lshr' || $opcode eq 'ashr' ) {
                        my ( $lhs, $rhs ) = $inst->operands->@*;
                        my $is_float = $lhs->type && $lhs->type->kind eq 'float';
                        my $mop = $is_float ? "f$opcode" : $opcode;
                        $mop = $opcode if $opcode eq 'shl' || $opcode eq 'lshr' || $opcode eq 'ashr';
                        ...
                        if ($is_float) {
                            ...
                            if ($is_i128 && ($opcode eq 'add' || $opcode eq 'sub' || $opcode eq 'and' || $opcode eq 'or' || $opcode eq 'xor' || $opcode eq 'shl' || $opcode eq 'lshr' || $opcode eq 'ashr' || $opcode eq 'mul' || $opcode eq 'div' || $opcode eq 'rem')) {
                                if ($opcode eq 'shl' || $opcode eq 'lshr' || $opcode eq 'ashr') {
                                    ...
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mul', operands => [$lo_dst, $lo_rhs], comment => 'i128 mul lo = a_lo * b_lo'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$hi_dst, $lo_lhs], comment => 'i128 mul hi'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mulhu', operands => [$hi_dst, $lo_rhs], comment => 'i128 mul hi = mulhu(a_lo, b_lo)'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$tmp1, $lo_lhs], comment => 'i128 mul tmp1'));
                                    if ($hi_rhs->kind eq 'imm') {
                                        my $r = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . '_rh', type => Brocken::Lindsay::IR::Type::i64());
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$r, $hi_rhs], comment => 'i128 mul rhs hi'));
                                    ...
                                    for my $ii (reverse 0 .. 127) {
                                        ...
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$t_hi_gt, $hi_rhs], comment => "i128 div hgt$ii load d_hi"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'sltu', operands => [$t_hi_gt, $r_hi], comment => "i128 div hgt$ii sltu"));
                                        # cond_hi_eq = r_hi == d_hi
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$t_hi_eq, $r_hi], comment => "i128 div heq$ii load"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'xor', operands => [$t_hi_eq, $hi_rhs], comment => "i128 div heq$ii xor"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'sltiu', operands => [$t_hi_eq, $i_one], comment => "i128 div heq$ii seqz"));
                                        # cond_lo_ge = r_lo >= d_lo  (i.e., !(r_lo < d_lo))
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$t_lo_ge, $r_lo], comment => "i128 div lge$ii load"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'sltu', operands => [$t_lo_ge, $lo_rhs], comment => "i128 div lge$ii sltu"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'xor', operands => [$t_lo_ge, $i_one], comment => "i128 div lge$ii not"));
                                        # cond = cond_hi_gt | (cond_hi_eq & cond_lo_ge)
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'and', operands => [$t_hi_eq, $t_lo_ge], comment => "i128 div cond$ii and"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'or', operands => [$t_hi_gt, $t_hi_eq], comment => "i128 div cond$ii or"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$t_cond, $t_hi_gt], comment => "i128 div cond$ii save"));
                                        ...
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$t_bor, $r_lo], comment => "i128 div bor$ii load"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'sltu', operands => [$t_bor, $t_mdl], comment => "i128 div bor$ii sltu"));
                                        # r_lo -= masked_d_lo
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$r_lo, $r_lo], comment => "i128 div r_lo$ii sub"));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'sub', operands => [$r_lo, $t_mdl], comment => "i128 div r_lo$ii sub"));
                                        # masked_d_hi = d_hi & neg_cond
                                ...
                                my ($lo_rhs, $hi_rhs) = $self->_split_i128($rhs);
                                my $tmp = Brocken::Jenny::MIR::MachineOperand->new(
                                    kind => 'virt_reg', value => $inst->name . '_tmp', type => Brocken::Lindsay::IR::Type::i64()
                                );
                                # lo part
                                $mbb->add_instruction(
                                    Brocken::Jenny::MIR::MachineInstruction->new(
                        ...
                        if ($inst->type && $inst->type->kind eq 'int' && $inst->type->bits == 128) {
                            ...
                            $mbb->add_instruction(
                                Brocken::Jenny::MIR::MachineInstruction->new(
                                    opcode => 'load', operands => [ $lo_dst, $lo_mem ], comment => 'i128 load lo'
                                )
                            );
                            $mbb->add_instruction(
                                Brocken::Jenny::MIR::MachineInstruction->new(
                                    opcode => 'load', operands => [ $hi_dst, $hi_mem ], comment => 'i128 load hi'
                                )
                            );
                        }
                        else {
                        ...
                        if ($val->type && $val->type->kind eq 'int' && $val->type->bits == 128) {
                            ...
                            $mbb->add_instruction(
                                Brocken::Jenny::MIR::MachineInstruction->new(
                                    opcode => ($lo_val->kind eq 'imm' ? 'store_imm' : 'store'), operands => [ $lo_mem, $lo_val ], comment => 'i128 store lo'
                                )
                            );
                            $mbb->add_instruction(
                                Brocken::Jenny::MIR::MachineInstruction->new(
                                    opcode => ($hi_val->kind eq 'imm' ? 'store_imm' : 'store'), operands => [ $hi_mem, $hi_val ], comment => 'i128 store hi'
                                )
                            );
                        }
                        else {
                        ...
                        if ( $idx->isa('Brocken::Lindsay::IR::Constant') ) {
                            ...
                            $mbb->add_instruction(
                                Brocken::Jenny::MIR::MachineInstruction->new(
                                    opcode => 'mv', operands => [ $dst, Brocken::Jenny::MIR::MachineOperand->new( kind => 'virt_reg', value => $ptr->name ) ],
                                    comment => 'gep: mv ptr'
                                )
                            );
                            $mbb->add_instruction(
                            ...
                            $mbb->add_instruction(
                                Brocken::Jenny::MIR::MachineInstruction->new(
                                    opcode => 'mv', operands => [ $dst, Brocken::Jenny::MIR::MachineOperand->new( kind => 'virt_reg', value => $idx->name ) ],
                                    comment => 'gep: mv idx'
                                )
                            );
                            ...
                            if ($log2scale > 0) {
                                $mbb->add_instruction(
                                    Brocken::Jenny::MIR::MachineInstruction->new(
                                        opcode => 'shl', operands => [ $dst, Brocken::Jenny::MIR::MachineOperand->new( kind => 'imm', value => $log2scale ) ],
                            ...
                            $mbb->add_instruction(
                                Brocken::Jenny::MIR::MachineInstruction->new(
                                    opcode => 'add', operands => [ $dst, Brocken::Jenny::MIR::MachineOperand->new( kind => 'virt_reg', value => $ptr->name ) ],
                                    comment => 'gep: add ptr'
                                )
                            );
                        }
                        ...
                        if ( $lhs->type && $lhs->type->kind eq 'float' ) {
                            ...
                            if ($pred eq 'eq' || $pred eq 'ne') {
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$t0, $lo_lhs], comment => 'i128 icmp lo'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'xor', operands => [$t0, $lo_rhs], comment => 'i128 icmp lo_diff'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$t1, $hi_lhs], comment => 'i128 icmp hi'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'xor', operands => [$t1, $hi_rhs], comment => 'i128 icmp hi_diff'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'or', operands => [$t0, $t1], comment => 'i128 icmp combined'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'sltiu', operands => [$dst, $i_one], comment => 'i128 icmp seqz'));
                                if ($pred eq 'ne') {
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'xor', operands => [$dst, $i_one], comment => 'i128 icmp ne'));
                                }
                            }
                            else {
                                ...
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$t0, $hi_a], comment => 'i128 icmp hi'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => $lt_op, operands => [$t0, $hi_b], comment => 'i128 icmp hi_' . $lt_op));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$t1, $hi_a], comment => 'i128 icmp hi_eq'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'xor', operands => [$t1, $hi_b], comment => 'i128 icmp hi_xor'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'sltiu', operands => [$t1, $i_one], comment => 'i128 icmp hi_eq'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$t2, $lo_a], comment => 'i128 icmp lo'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'sltu', operands => [$t2, $lo_b], comment => 'i128 icmp lo_lt'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'and', operands => [$t1, $t2], comment => 'i128 icmp hi_eq&lo_lt'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'or', operands => [$t0, $t1], comment => 'i128 icmp result'));
                                if ($pred eq 'ule' || $pred eq 'uge' || $pred eq 'sle' || $pred eq 'sge') {
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'xor', operands => [$t0, $i_one], comment => 'i128 icmp ' . $pred));
                                }
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'mv', operands => [$dst, $t0], comment => 'i128 icmp store'));
                            }
                        }
    ...
    # ---------------------------------------------------------------------------
     class Brocken::Jenny::Lowerer::Wasm {
        method lower($ir_func) {
            ...
            for my $block ( $ir_func->blocks->@* ) {
                ...
                for my $inst ( $block->instructions->@* ) {
                    ...
                    if ( $opcode eq 'add' || $opcode eq 'sub' || $opcode eq 'mul' || $opcode eq 'div' || $opcode eq 'rem' || $opcode eq 'and' || $opcode eq 'or' || $opcode eq 'xor' || $opcode eq 'shl' || $opcode eq 'lshr' || $opcode eq 'ashr' || $opcode eq 'min' || $opcode eq 'max' ) {
                        my ( $lhs, $rhs ) = $inst->operands->@*;
                        if ($inst->type && $inst->type->kind eq 'int' && $inst->type->bits == 128 && ($opcode eq 'add' || $opcode eq 'sub' || $opcode eq 'and' || $opcode eq 'or' || $opcode eq 'xor' || $opcode eq 'shl' || $opcode eq 'lshr' || $opcode eq 'ashr' || $opcode eq 'mul' || $opcode eq 'div' || $opcode eq 'rem')) {
                            if ($opcode eq 'shl' || $opcode eq 'lshr' || $opcode eq 'ashr') {
                                my ($lo_lhs, $hi_lhs) = $self->_split_i128($lhs);
                                my ($lo_dst, $hi_dst) = $self->_split_i128($inst);
                                my $lo_tmp = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . '_t', type => Brocken::Lindsay::IR::Type::i64());
                                if ($rhs->isa('Brocken::Lindsay::IR::Constant')) {
                                    ...
                                    if ($amt == 0) {
                                        ...
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$lo_dst], comment => 'store lo'));
                                        $mbb->add_instruction($self->_wasm_push_opnd($hi_lhs, 'hi_lhs'));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$hi_dst], comment => 'store hi'));
                                    }
                                    elsif ($opcode eq 'shl') {
                                        if ($amt < 64) {
                                            $mbb->add_instruction($self->_wasm_push_opnd($lo_lhs, 'lo_lhs'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_const', operands => [Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => $amt)], comment => 'amt ' . $amt));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_shl', operands => [], comment => 'lo<<' . $amt));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$lo_dst], comment => 'store lo'));
                                            $mbb->add_instruction($self->_wasm_push_opnd($lo_lhs, 'lo_lhs'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_const', operands => [Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 64 - $amt)], comment => 'amt ' . (64 - $amt)));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_shr_u', operands => [], comment => 'carry>>' . (64 - $amt)));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$lo_tmp], comment => 'save carry'));
                                            $mbb->add_instruction($self->_wasm_push_opnd($hi_lhs, 'hi_lhs'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_const', operands => [Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => $amt)], comment => 'amt ' . $amt));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_shl', operands => [], comment => 'hi<<' . $amt));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_get', operands => [$lo_tmp], comment => 'push carry'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_or', operands => [], comment => 'hi|=carry'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$hi_dst], comment => 'store hi'));
                                        }
                                        elsif ($amt == 64) {
                                            $mbb->add_instruction($self->_wasm_push_opnd($lo_lhs, 'lo_lhs'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$hi_dst], comment => 'store hi=lo'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_const', operands => [Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0)], comment => '0'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$lo_dst], comment => 'store lo=0'));
                                        }
                                        elsif ($amt < 128) {
                                            $mbb->add_instruction($self->_wasm_push_opnd($lo_lhs, 'lo_lhs'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_const', operands => [Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => $amt - 64)], comment => 'amt ' . ($amt - 64)));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_shl', operands => [], comment => 'hi<<' . ($amt - 64)));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$hi_dst], comment => 'store hi'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_const', operands => [Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0)], comment => '0'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$lo_dst], comment => 'store lo=0'));
                                        }
                                            ...
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$lo_dst], comment => 'store lo=0'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_const', operands => [Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0)], comment => '0'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$hi_dst], comment => 'store hi=0'));
                                        }
                                    }
                                    elsif ($opcode eq 'lshr') {
                                        if ($amt < 64) {
                                            $mbb->add_instruction($self->_wasm_push_opnd($hi_lhs, 'hi_lhs'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_const', operands => [Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => $amt)], comment => 'amt ' . $amt));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_shr_u', operands => [], comment => 'hi>>' . $amt));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$hi_dst], comment => 'store hi'));
                                            $mbb->add_instruction($self->_wasm_push_opnd($hi_lhs, 'hi_lhs'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_const', operands => [Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 64 - $amt)], comment => 'amt ' . (64 - $amt)));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_shl', operands => [], comment => 'carry<<' . (64 - $amt)));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$lo_tmp], comment => 'save carry'));
                                            $mbb->add_instruction($self->_wasm_push_opnd($lo_lhs, 'lo_lhs'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_const', operands => [Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => $amt)], comment => 'amt ' . $amt));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_shr_u', operands => [], comment => 'lo>>' . $amt));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_get', operands => [$lo_tmp], comment => 'push carry'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_or', operands => [], comment => 'lo|=carry'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$lo_dst], comment => 'store lo'));
                                        }
                                        elsif ($amt == 64) {
                                            $mbb->add_instruction($self->_wasm_push_opnd($hi_lhs, 'hi_lhs'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$lo_dst], comment => 'store lo=hi'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_const', operands => [Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0)], comment => '0'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$hi_dst], comment => 'store hi=0'));
                                        }
                                        elsif ($amt < 128) {
                                            $mbb->add_instruction($self->_wasm_push_opnd($hi_lhs, 'hi_lhs'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_const', operands => [Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => $amt - 64)], comment => 'amt ' . ($amt - 64)));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_shr_u', operands => [], comment => 'lo>>' . ($amt - 64)));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$lo_dst], comment => 'store lo'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_const', operands => [Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0)], comment => '0'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$hi_dst], comment => 'store hi=0'));
                                        }
                                        ...
                                        if ($amt < 64) {
                                            $mbb->add_instruction($self->_wasm_push_opnd($hi_lhs, 'hi_lhs'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_const', operands => [Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => $amt)], comment => 'amt ' . $amt));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_shr_s', operands => [], comment => 'hi>>' . $amt));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$hi_dst], comment => 'store hi'));
                                            $mbb->add_instruction($self->_wasm_push_opnd($hi_lhs, 'hi_lhs'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_const', operands => [Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 64 - $amt)], comment => 'amt ' . (64 - $amt)));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_shl', operands => [], comment => 'carry<<' . (64 - $amt)));
                                            ...
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$lo_dst], comment => 'store lo=hi'));
                                            $mbb->add_instruction($self->_wasm_push_opnd($hi_lhs, 'hi_lhs'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_const', operands => [Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 63)], comment => '63'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_shr_s', operands => [], comment => 'sign extend'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$hi_dst], comment => 'store hi sign'));
                                        }
                                        elsif ($amt < 128) {
                                            $mbb->add_instruction($self->_wasm_push_opnd($hi_lhs, 'hi_lhs'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_const', operands => [Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => $amt - 64)], comment => 'amt ' . ($amt - 64)));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_shr_s', operands => [], comment => 'lo>>' . ($amt - 64)));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$lo_dst], comment => 'store lo'));
                                            $mbb->add_instruction($self->_wasm_push_opnd($hi_lhs, 'hi_lhs'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_const', operands => [Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 63)], comment => '63'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_shr_s', operands => [], comment => 'sign extend'));
                                            ...
                                            $mbb->add_instruction($self->_wasm_push_opnd($hi_lhs, 'hi_lhs'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_const', operands => [Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 63)], comment => '63'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_shr_s', operands => [], comment => 'sign'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$lo_tmp], comment => 'save sign'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_get', operands => [$lo_tmp], comment => 'push sign'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$lo_dst], comment => 'store lo'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_get', operands => [$lo_tmp], comment => 'push sign'));
                                            $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$hi_dst], comment => 'store hi'));
                                        }
                                ...
                                my ($lo_rhs, $hi_rhs) = $self->_split_i128($rhs);
                                my ($lo_dst, $hi_dst) = $self->_split_i128($inst);
                                my $al = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . '_al', type => Brocken::Lindsay::IR::Type::i64());
                                my $ah = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . '_ah', type => Brocken::Lindsay::IR::Type::i64());
                                my $bl = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . '_bl', type => Brocken::Lindsay::IR::Type::i64());
                                my $bh = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . '_bh', type => Brocken::Lindsay::IR::Type::i64());
                                my $p0 = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . '_p0', type => Brocken::Lindsay::IR::Type::i64());
                                my $p1 = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . '_p1', type => Brocken::Lindsay::IR::Type::i64());
                                my $p2 = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . '_p2', type => Brocken::Lindsay::IR::Type::i64());
                                my $p3 = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . '_p3', type => Brocken::Lindsay::IR::Type::i64());
                                my $carry = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . '_carry', type => Brocken::Lindsay::IR::Type::i64());
                                # umulh(a_lo, b_lo) via 32-bit schoolbook decomposition
                                # al = a_lo & 0xFFFFFFFF
                                $mbb->add_instruction($self->_wasm_push_opnd($lo_lhs, 'lo_lhs'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_const', operands => [Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0xFFFFFFFF)], comment => 'mask'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_and', operands => [], comment => 'al'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$al], comment => 'save al'));
                                # ah = a_lo >> 32
                                $mbb->add_instruction($self->_wasm_push_opnd($lo_lhs, 'lo_lhs'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_const', operands => [Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 32)], comment => '32'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_shr_u', operands => [], comment => 'ah'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$ah], comment => 'save ah'));
                                # bl = b_lo & 0xFFFFFFFF
                                $mbb->add_instruction($self->_wasm_push_opnd($lo_rhs, 'lo_rhs'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_const', operands => [Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0xFFFFFFFF)], comment => 'mask'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_and', operands => [], comment => 'bl'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$bl], comment => 'save bl'));
                                # bh = b_lo >> 32
                                $mbb->add_instruction($self->_wasm_push_opnd($lo_rhs, 'lo_rhs'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_const', operands => [Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 32)], comment => '32'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_shr_u', operands => [], comment => 'bh'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$bh], comment => 'save bh'));
                                # p0 = al * bl
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_get', operands => [$al], comment => 'al'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_get', operands => [$bl], comment => 'bl'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_mul', operands => [], comment => 'p0'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$p0], comment => 'save p0'));
                                # p1 = al * bh
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_get', operands => [$al], comment => 'al'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_get', operands => [$bh], comment => 'bh'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_mul', operands => [], comment => 'p1'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$p1], comment => 'save p1'));
                                # p2 = ah * bl
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_get', operands => [$ah], comment => 'ah'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_get', operands => [$bl], comment => 'bl'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_mul', operands => [], comment => 'p2'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$p2], comment => 'save p2'));
                                # p3 = ah * bh
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_get', operands => [$ah], comment => 'ah'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_get', operands => [$bh], comment => 'bh'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_mul', operands => [], comment => 'p3'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$p3], comment => 'save p3'));
                                # carry = (p0>>32) + (p1&0xFFFFFFFF) + (p2&0xFFFFFFFF)
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_get', operands => [$p0], comment => 'p0'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_const', operands => [Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 32)], comment => '32'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_shr_u', operands => [], comment => '>>32'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_get', operands => [$p1], comment => 'p1'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_const', operands => [Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0xFFFFFFFF)], comment => 'mask'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_and', operands => [], comment => '&mask'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_add', operands => [], comment => '+'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_get', operands => [$p2], comment => 'p2'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_const', operands => [Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0xFFFFFFFF)], comment => 'mask'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_and', operands => [], comment => '&mask'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_add', operands => [], comment => 'carry0'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$carry], comment => 'save carry0'));
                                # carry = carry0 >> 32  (carry1)
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_get', operands => [$carry], comment => 'carry0'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_const', operands => [Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 32)], comment => '32'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_shr_u', operands => [], comment => 'carry1'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$carry], comment => 'save carry1'));
                                ...
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_shr_u', operands => [], comment => '>>32'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_add', operands => [], comment => '+'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_get', operands => [$p3], comment => 'p3'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_add', operands => [], comment => '+p3'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_get', operands => [$carry], comment => 'carry1'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_add', operands => [], comment => 'umulh(a_lo,b_lo)'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$hi_dst], comment => 'save hi (umulh)'));
                                # lo_dst = a_lo * b_lo (low 64 bits)
                                $mbb->add_instruction($self->_wasm_push_opnd($lo_lhs, 'lo_lhs'));
                                $mbb->add_instruction($self->_wasm_push_opnd($lo_rhs, 'lo_rhs'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_mul', operands => [], comment => 'lo = a_lo*b_lo'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$lo_dst], comment => 'save lo'));
                                # hi_dst += a_lo * b_hi
                                $mbb->add_instruction($self->_wasm_push_opnd($lo_lhs, 'lo_lhs'));
                                $mbb->add_instruction($self->_wasm_push_opnd($hi_rhs, 'hi_rhs'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_mul', operands => [], comment => 'a_lo*b_hi'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_get', operands => [$hi_dst], comment => 'hi'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_add', operands => [], comment => 'hi += a_lo*b_hi'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$hi_dst], comment => 'save hi'));
                                # hi_dst += a_hi * b_lo
                                $mbb->add_instruction($self->_wasm_push_opnd($hi_lhs, 'hi_lhs'));
                                $mbb->add_instruction($self->_wasm_push_opnd($lo_rhs, 'lo_rhs'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_mul', operands => [], comment => 'a_hi*b_lo'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_get', operands => [$hi_dst], comment => 'hi'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_add', operands => [], comment => 'hi += a_hi*b_lo'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$hi_dst], comment => 'save hi'));
                            }
                            elsif ($opcode eq 'div' || $opcode eq 'rem') {
                                my ($lo_lhs, $hi_lhs) = $self->_split_i128($lhs);
                                ...
                                my $r_hi = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . '_r_hi', type => Brocken::Lindsay::IR::Type::i64());
                                # Q = 0, R = 0
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_const', operands => [Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 0)]));
                                ...
                                for my $ii (reverse 0 .. 127) {
                                    ...
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_const', operands => [Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => $shift)]));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_shr_u', operands => []));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_const', operands => [Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 1)]));
                                    ...
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_const', operands => [Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => 63)]));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_shr_u', operands => []));
                                    my $carry = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . "_c$ii", type => Brocken::Lindsay::IR::Type::i64());
                                    ...
                                    my $cond_hi_gt = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . "_hgt$ii", type => Brocken::Lindsay::IR::Type::i32());
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$cond_hi_gt]));
                                    # cond_hi_eq = r_hi == d_hi
                                    $mbb->add_instruction($self->_wasm_push_opnd($r_hi, 'r_hi'));
                                    $mbb->add_instruction($self->_wasm_push_opnd($hi_rhs, 'd_hi'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_eq', operands => []));
                                    my $cond_hi_eq = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . "_heq$ii", type => Brocken::Lindsay::IR::Type::i32());
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$cond_hi_eq]));
                                    ...
                                    my $cond_lo_ge = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . "_lge$ii", type => Brocken::Lindsay::IR::Type::i32());
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$cond_lo_ge]));
                                    # cond = cond_hi_gt | (cond_hi_eq & cond_lo_ge)
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_get', operands => [$cond_hi_eq]));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_get', operands => [$cond_lo_ge]));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i32_and', operands => []));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_get', operands => [$cond_hi_gt]));
                                    ...
                                    my $cond = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . "_cond$ii", type => Brocken::Lindsay::IR::Type::i64());
                                    ...
                                    my $neg_cond = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . "_neg$ii", type => Brocken::Lindsay::IR::Type::i64());
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$neg_cond]));
                                    # masked_d_lo = d_lo & neg_cond
                                    $mbb->add_instruction($self->_wasm_push_opnd($lo_rhs, 'd_lo'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_get', operands => [$neg_cond]));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_and', operands => []));
                                    my $masked_d_lo = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . "_mdl$ii", type => Brocken::Lindsay::IR::Type::i64());
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$masked_d_lo]));
                                    # borrow = r_lo < masked_d_lo
                                    $mbb->add_instruction($self->_wasm_push_opnd($r_lo, 'r_lo'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_get', operands => [$masked_d_lo]));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_lt_u', operands => []));
                                    my $borrow = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . "_bor$ii", type => Brocken::Lindsay::IR::Type::i32());
                                    ...
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_get', operands => [$neg_cond]));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_and', operands => []));
                                    my $masked_d_hi = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . "_mdh$ii", type => Brocken::Lindsay::IR::Type::i64());
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$masked_d_hi]));
                                    # r_hi -= masked_d_hi + borrow
                                    $mbb->add_instruction($self->_wasm_push_opnd($r_hi, 'r_hi'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_get', operands => [$masked_d_hi]));
                                    ...
                                    # Q_bit = (1 << ($ii % 64)) & neg_cond
                                    my $qbit_val = 1 << ($ii % 64);
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_const', operands => [Brocken::Jenny::MIR::MachineOperand->new(kind => 'imm', value => $qbit_val)]));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_get', operands => [$neg_cond]));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_and', operands => []));
                                    my $qbit = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . "_qb$ii", type => Brocken::Lindsay::IR::Type::i64());
                                    ...
                                    if ($ii >= 64) {
                                        ...
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_get', operands => [$qbit]));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_or', operands => []));
                                        $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$q_hi]));
                                    }
                                    else {
                                ...
                                if ($opcode eq 'add') {
                                    $mbb->add_instruction($self->_wasm_push_opnd($lo_lhs, 'lo_lhs'));
                                    $mbb->add_instruction($self->_wasm_push_opnd($lo_rhs, 'lo_rhs'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_add', operands => [], comment => 'lo add'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$lo_dst], comment => 'store lo'));
                                    $mbb->add_instruction($self->_wasm_push_opnd($lo_lhs, 'lo_lhs'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_get', operands => [$lo_dst], comment => 'push lo_dst'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_gt_u', operands => [], comment => 'carry'));
                                    my $carry = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . '_carry', type => Brocken::Lindsay::IR::Type::i32());
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$carry], comment => 'save carry'));
                                    $mbb->add_instruction($self->_wasm_push_opnd($hi_lhs, 'hi_lhs'));
                                    $mbb->add_instruction($self->_wasm_push_opnd($hi_rhs, 'hi_rhs'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_add', operands => [], comment => 'hi add'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_get', operands => [$carry], comment => 'push carry'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_extend_i32_u', operands => [], comment => 'carry i32->i64'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_add', operands => [], comment => 'hi add carry'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$hi_dst], comment => 'store hi'));
                                }
                                elsif ($opcode eq 'sub') {
                                    $mbb->add_instruction($self->_wasm_push_opnd($lo_lhs, 'lo_lhs'));
                                    $mbb->add_instruction($self->_wasm_push_opnd($lo_rhs, 'lo_rhs'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_sub', operands => [], comment => 'lo sub'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$lo_dst], comment => 'store lo'));
                                    $mbb->add_instruction($self->_wasm_push_opnd($lo_lhs, 'lo_lhs'));
                                    $mbb->add_instruction($self->_wasm_push_opnd($lo_rhs, 'lo_rhs'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_lt_u', operands => [], comment => 'borrow'));
                                    my $borrow = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . '_carry', type => Brocken::Lindsay::IR::Type::i32());
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$borrow], comment => 'save borrow'));
                                    $mbb->add_instruction($self->_wasm_push_opnd($hi_lhs, 'hi_lhs'));
                                    $mbb->add_instruction($self->_wasm_push_opnd($hi_rhs, 'hi_rhs'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_sub', operands => [], comment => 'hi sub'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_get', operands => [$borrow], comment => 'push borrow'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_extend_i32_u', operands => [], comment => 'borrow i32->i64'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_sub', operands => [], comment => 'hi sub borrow'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$hi_dst], comment => 'store hi'));
                                }
                                else {
                                    $mbb->add_instruction($self->_wasm_push_opnd($lo_lhs, 'lo_lhs'));
                                    $mbb->add_instruction($self->_wasm_push_opnd($lo_rhs, 'lo_rhs'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => "i64_$opcode", operands => [], comment => "lo $opcode"));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$lo_dst], comment => 'store lo'));
                                    $mbb->add_instruction($self->_wasm_push_opnd($hi_lhs, 'hi_lhs'));
                                    $mbb->add_instruction($self->_wasm_push_opnd($hi_rhs, 'hi_rhs'));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => "i64_$opcode", operands => [], comment => "hi $opcode"));
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$hi_dst], comment => 'store hi'));
                                }
                            }
                        }
                        ...
                        my %map = (
                            add => "${p}_add", sub => "${p}_sub", mul => "${p}_mul", div => "${p}_div_u", rem => "${p}_rem_u",
                        ...
                        if ($inst->type && $inst->type->kind eq 'int' && $inst->type->bits == 128) {
                            ...
                            $mbb->add_instruction(
                                Brocken::Jenny::MIR::MachineInstruction->new(
                                    opcode => 'local_set', operands => [$lo_dst], comment => 'load lo'
                                )
                            );
                            $mbb->add_instruction( $self->_wasm_push($ptr, 'load: ptr') );
                            $mbb->add_instruction(
                            ...
                            $mbb->add_instruction(
                                Brocken::Jenny::MIR::MachineInstruction->new(
                                    opcode => 'local_set', operands => [$hi_dst], comment => 'load hi'
                                )
                            );
                        }
                        else {
                        ...
                        if ($float) {
                            ...
                            my ($lo_lhs, $hi_lhs) = $self->_split_i128($lhs);
                            my ($lo_rhs, $hi_rhs) = $self->_split_i128($rhs);
                            my $t0 = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . '_t0', type => Brocken::Lindsay::IR::Type::i32());
                            my $t1 = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name . '_t1', type => Brocken::Lindsay::IR::Type::i32());
                            my $dst = Brocken::Jenny::MIR::MachineOperand->new(kind => 'virt_reg', value => $inst->name, type => $inst->type);
                            if ($pred eq 'eq' || $pred eq 'ne') {
                                ...
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_xor', operands => [], comment => 'i128 icmp lo_xor'));
                                ...
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_xor', operands => [], comment => 'i128 icmp hi_xor'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_or', operands => [], comment => 'i128 icmp or'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_eqz', operands => [], comment => 'i128 icmp eqz'));
                                if ($pred eq 'ne') {
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i32_eqz', operands => [], comment => 'i128 icmp ne'));
                                }
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$dst], comment => 'i128 icmp store'));
                            }
                            else {
                                ...
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => $cmp, operands => [], comment => 'i128 icmp hi_' . $cmp));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$t0], comment => 'i128 icmp save hi_lt'));
                                $mbb->add_instruction($self->_wasm_push_opnd($hi_a, 'i128 icmp hi_a'));
                                $mbb->add_instruction($self->_wasm_push_opnd($hi_b, 'i128 icmp hi_b'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_eq', operands => [], comment => 'i128 icmp hi_eq'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$t1], comment => 'i128 icmp save hi_eq'));
                                $mbb->add_instruction($self->_wasm_push_opnd($lo_a, 'i128 icmp lo_a'));
                                $mbb->add_instruction($self->_wasm_push_opnd($lo_b, 'i128 icmp lo_b'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i64_lt_u', operands => [], comment => 'i128 icmp lo_lt'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_get', operands => [$t1], comment => 'i128 icmp push hi_eq'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i32_and', operands => [], comment => 'i128 icmp hi_eq&lo_lt'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_get', operands => [$t0], comment => 'i128 icmp push hi_lt'));
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i32_or', operands => [], comment => 'i128 icmp result'));
                                if ($pred eq 'ule' || $pred eq 'uge' || $pred eq 'sle' || $pred eq 'sge') {
                                    $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'i32_eqz', operands => [], comment => 'i128 icmp invert'));
                                }
                                $mbb->add_instruction(Brocken::Jenny::MIR::MachineInstruction->new(opcode => 'local_set', operands => [$dst], comment => 'i128 icmp store'));
                            }
                            next;
        ...
        method _wasm_push_opnd( $opnd, $label ) {
            if ($opnd->kind eq 'imm') {
                return Brocken::Jenny::MIR::MachineInstruction->new(
                    opcode => 'i64_const', operands => [$opnd],
                    comment => "push $label=" . $opnd->value
                );
            }
            return Brocken::Jenny::MIR::MachineInstruction->new(
                opcode => 'local_get', operands => [$opnd],
                comment => "push $label=" . $opnd->value
            );
        }
...
subtest Jenny => sub {
    ...
    subtest 'Jenny::Codegen GEP Lowering' => sub {
        my $host = Brocken::Katsuro::Platform::parse();
        {
            ...
            my $gep = $builder->build_gep( Brocken::Lindsay::IR::Type::i32(), $ptr, [ Brocken::Lindsay::IR::Constant->new( type => Brocken::Lindsay::IR::Type::i32(), value => 42 ) ], '%elem' );
            $builder->build_ret($gep);
            my $lowerer = Brocken::Jenny::Lowerer::X86_64->new();
...
subtest 'Jenny::Codegen i128 Lowering' => sub {
    ...
    $check->($X, 'i128_add', sub {
        my $b = shift; $b->build_ret($b->build_add(
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 10),
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 5), '%r'));
    }, adc => 'x86_64 i128 add: adc produced', add => 'x86_64 i128 add: add for lo');
    $check->($X, 'i128_sub', sub {
        my $b = shift; $b->build_ret($b->build_sub(
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 10),
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 5), '%r'));
    }, sbb => 'x86_64 i128 sub: sbb produced');
    $check->($X, 'i128_and', sub {
        my $b = shift; $b->build_ret($b->build_and(
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 0xFFFFFFFFFFFFFFFF),
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 0x123456789ABCDEF0), '%r'));
    }, and => 'x86_64 i128 and: and produced');
    $check->($X, 'i128_or', sub {
        my $b = shift; $b->build_ret($b->build_or(
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 0xFFFFFFFFFFFFFFFF),
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 0x123456789ABCDEF0), '%r'));
    }, or => 'x86_64 i128 or: or produced');
    $check->($X, 'i128_xor', sub {
        my $b = shift; $b->build_ret($b->build_xor(
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 0xFFFFFFFFFFFFFFFF),
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 0x123456789ABCDEF0), '%r'));
    }, xor => 'x86_64 i128 xor: xor produced');
    $check->($A, 'i128_add', sub {
        my $b = shift; $b->build_ret($b->build_add(
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 10),
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 5), '%r'));
    }, sltu => 'ARM64 i128 add: sltu carry produced');
    $check->($A, 'i128_sub', sub {
        my $b = shift; $b->build_ret($b->build_sub(
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 10),
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 5), '%r'));
    }, sltu => 'ARM64 i128 sub: sltu borrow produced');
    $check->($R, 'i128_add', sub {
        my $b = shift; $b->build_ret($b->build_add(
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 10),
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 5), '%r'));
    }, sltu => 'RISCV64 i128 add: sltu carry produced');
    $check->($R, 'i128_sub', sub {
        my $b = shift; $b->build_ret($b->build_sub(
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 10),
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 5), '%r'));
    }, sltu => 'RISCV64 i128 sub: sltu borrow produced');
    $check->($W, 'i128_add', sub {
        my $b = shift; $b->build_ret($b->build_add(
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 10),
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 5), '%r'));
    }, i64_gt_u => 'Wasm i128 add: i64_gt_u carry produced');
    $check->($W, 'i128_sub', sub {
        my $b = shift; $b->build_ret($b->build_sub(
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 10),
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 5), '%r'));
    }, i64_lt_u => 'Wasm i128 sub: i64_lt_u borrow produced');
    $check->($W, 'i128_and', sub {
        my $b = shift; $b->build_ret($b->build_and(
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 0xFFFFFFFFFFFFFFFF),
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 0x123456789ABCDEF0), '%r'));
    ...
    $check->($X, 'i128_shl_1', sub {
        my $b = shift; $b->build_ret($b->build_shl(
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 0x123456789ABCDEF0),
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i64(), value => 1), '%r'));
    }, shl => 'x86_64 i128 shl/1: shl produced', lshr => 'x86_64 i128 shl/1: lshr carry produced', or => 'x86_64 i128 shl/1: or carry merge');
    # x86_64 i128 lshr lowering (constant amt=1)
    $check->($X, 'i128_lshr_1', sub {
        my $b = shift; $b->build_ret($b->build_lshr(
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 0x123456789ABCDEF0),
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i64(), value => 1), '%r'));
    }, lshr => 'x86_64 i128 lshr/1: lshr produced', shl => 'x86_64 i128 lshr/1: shl carry produced', or => 'x86_64 i128 lshr/1: or carry merge');
    # x86_64 i128 ashr lowering (constant amt=1)
    $check->($X, 'i128_ashr_1', sub {
        my $b = shift; $b->build_ret($b->build_ashr(
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 0x123456789ABCDEF0),
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i64(), value => 1), '%r'));
    }, ashr => 'x86_64 i128 ashr/1: ashr produced', shl => 'x86_64 i128 ashr/1: shl carry produced', or => 'x86_64 i128 ashr/1: or carry merge');
    # ARM64 i128 shl lowering (constant amt=1)
    $check->($A, 'i128_shl_1', sub {
        my $b = shift; $b->build_ret($b->build_shl(
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 0x123456789ABCDEF0),
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i64(), value => 1), '%r'));
    }, shl => 'ARM64 i128 shl/1: shl produced', lshr => 'ARM64 i128 shl/1: lshr carry produced', or => 'ARM64 i128 shl/1: or carry merge');
    # ARM64 i128 lshr lowering (constant amt=1)
    $check->($A, 'i128_lshr_1', sub {
        my $b = shift; $b->build_ret($b->build_lshr(
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 0x123456789ABCDEF0),
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i64(), value => 1), '%r'));
    }, lshr => 'ARM64 i128 lshr/1: lshr produced', shl => 'ARM64 i128 lshr/1: shl carry produced', or => 'ARM64 i128 lshr/1: or carry merge');
    # ARM64 i128 ashr lowering (constant amt=1)
    $check->($A, 'i128_ashr_1', sub {
        my $b = shift; $b->build_ret($b->build_ashr(
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 0x123456789ABCDEF0),
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i64(), value => 1), '%r'));
    }, ashr => 'ARM64 i128 ashr/1: ashr produced', shl => 'ARM64 i128 ashr/1: shl carry produced', or => 'ARM64 i128 ashr/1: or carry merge');
    # RISCV64 i128 shl lowering (constant amt=1)
    $check->($R, 'i128_shl_1', sub {
        my $b = shift; $b->build_ret($b->build_shl(
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 0x123456789ABCDEF0),
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i64(), value => 1), '%r'));
    }, shl => 'RISCV64 i128 shl/1: shl produced', lshr => 'RISCV64 i128 shl/1: lshr carry produced', or => 'RISCV64 i128 shl/1: or carry merge');
    # RISCV64 i128 lshr lowering (constant amt=1)
    $check->($R, 'i128_lshr_1', sub {
        my $b = shift; $b->build_ret($b->build_lshr(
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 0x123456789ABCDEF0),
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i64(), value => 1), '%r'));
    }, lshr => 'RISCV64 i128 lshr/1: lshr produced', shl => 'RISCV64 i128 lshr/1: shl carry produced', or => 'RISCV64 i128 lshr/1: or carry merge');
    # RISCV64 i128 ashr lowering (constant amt=1)
    $check->($R, 'i128_ashr_1', sub {
        my $b = shift; $b->build_ret($b->build_ashr(
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 0x123456789ABCDEF0),
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i64(), value => 1), '%r'));
    }, ashr => 'RISCV64 i128 ashr/1: ashr produced', shl => 'RISCV64 i128 ashr/1: shl carry produced', or => 'RISCV64 i128 ashr/1: or carry merge');
    # Wasm i128 shl lowering (constant amt=1)
    $check->($W, 'i128_shl_1', sub {
        my $b = shift; $b->build_ret($b->build_shl(
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 0x123456789ABCDEF0),
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i64(), value => 1), '%r'));
    }, i64_shl => 'Wasm i128 shl/1: i64_shl produced', i64_shr_u => 'Wasm i128 shl/1: i64_shr_u carry produced', i64_or => 'Wasm i128 shl/1: i64_or carry merge');
    # Wasm i128 lshr lowering (constant amt=1)
    $check->($W, 'i128_lshr_1', sub {
        my $b = shift; $b->build_ret($b->build_lshr(
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 0x123456789ABCDEF0),
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i64(), value => 1), '%r'));
    }, i64_shr_u => 'Wasm i128 lshr/1: i64_shr_u produced', i64_shl => 'Wasm i128 lshr/1: i64_shl carry produced', i64_or => 'Wasm i128 lshr/1: i64_or carry merge');
    # Wasm i128 ashr lowering (constant amt=1)
    $check->($W, 'i128_ashr_1', sub {
        my $b = shift; $b->build_ret($b->build_ashr(
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 0x123456789ABCDEF0),
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i64(), value => 1), '%r'));
    }, i64_shr_s => 'Wasm i128 ashr/1: i64_shr_s produced', i64_shl => 'Wasm i128 ashr/1: i64_shl carry produced', i64_or => 'Wasm i128 ashr/1: i64_or carry merge');
    # x86_64 i128 mul lowering
    $check->($X, 'i128_mul', sub {
        my $b = shift; $b->build_ret($b->build_mul(
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 42),
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 2), '%r'));
    }, mul => 'x86_64 i128 mul: mul produced', umulh => 'x86_64 i128 mul: umulh produced');
    # ARM64 i128 mul lowering
    $check->($A, 'i128_mul', sub {
        my $b = shift; $b->build_ret($b->build_mul(
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 42),
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 2), '%r'));
    }, mul => 'ARM64 i128 mul: mul produced', umulh => 'ARM64 i128 mul: umulh produced');
    # RISCV64 i128 mul lowering
    $check->($R, 'i128_mul', sub {
        my $b = shift; $b->build_ret($b->build_mul(
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 42),
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 2), '%r'));
    }, mul => 'RISCV64 i128 mul: mul produced', mulhu => 'RISCV64 i128 mul: mulhu produced');
    # Wasm i128 mul lowering
    $check->($W, 'i128_mul', sub {
        my $b = shift; $b->build_ret($b->build_mul(
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 42),
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 2), '%r'));
    }, i64_mul => 'Wasm i128 mul: i64_mul produced', i64_and => 'Wasm i128 mul: i64_and produced', i64_shr_u => 'Wasm i128 mul: i64_shr_u produced');
    # x86_64 i128 div lowering
    $check->($X, 'i128_div', sub {
        my $b = shift; $b->build_ret($b->build_div(
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 42),
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 2), '%r'));
    }, seta => 'x86_64 i128 div: seta produced', setae => 'x86_64 i128 div: setae produced');
    # x86_64 i128 rem lowering
    $check->($X, 'i128_rem', sub {
        my $b = shift; $b->build_ret($b->build_rem(
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 42),
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 2), '%r'));
    }, setb => 'x86_64 i128 rem: setb produced', seta => 'x86_64 i128 rem: seta produced');
    # ARM64 i128 div lowering
    $check->($A, 'i128_div', sub {
        my $b = shift; $b->build_ret($b->build_div(
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 42),
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 2), '%r'));
    }, cset_hi => 'ARM64 i128 div: cset_hi produced', cset_cs => 'ARM64 i128 div: cset_cs produced');
    # ARM64 i128 rem lowering
    $check->($A, 'i128_rem', sub {
        my $b = shift; $b->build_ret($b->build_rem(
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 42),
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 2), '%r'));
    }, cset_hi => 'ARM64 i128 rem: cset_hi produced', cset_cc => 'ARM64 i128 rem: cset_cc produced');
    # RISCV64 i128 div lowering
    $check->($R, 'i128_div', sub {
        my $b = shift; $b->build_ret($b->build_div(
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 42),
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 2), '%r'));
    }, sltu => 'RISCV64 i128 div: sltu produced');
    # RISCV64 i128 rem lowering
    $check->($R, 'i128_rem', sub {
        my $b = shift; $b->build_ret($b->build_rem(
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 42),
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 2), '%r'));
    }, sltu => 'RISCV64 i128 rem: sltu produced');
    # Wasm i128 div lowering
    $check->($W, 'i128_div', sub {
        my $b = shift; $b->build_ret($b->build_div(
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 42),
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 2), '%r'));
    }, i64_sub => 'Wasm i128 div: i64_sub produced');
    # Wasm i128 rem lowering
    $check->($W, 'i128_rem', sub {
        my $b = shift; $b->build_ret($b->build_rem(
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 42),
            Brocken::Lindsay::IR::Constant->new(type => Brocken::Lindsay::IR::Type::i128(), value => 2), '%r'));
...
subtest 'Jenny::Codegen i128 Arithmetic (Cross-Platform)' => sub {
    ...
    SKIP: {
        ...
        # Test 7: i128 shl (21 << 1 = 42)
        {
            ...
            my $builder = Brocken::Lindsay::IR::Builder->new();
            $builder->position_at_end( $func->append_block('entry') );
            $builder->build_ret( $builder->build_shl(
        ...
        # Test 8: i128 lshr (84 >> 1 = 42)
        {
            ...
            my $builder = Brocken::Lindsay::IR::Builder->new();
            $builder->position_at_end( $func->append_block('entry') );
            $builder->build_ret( $builder->build_lshr(
        ...
        # Test 9: i128 ashr (84 >> 1 = 42, positive)
        {
            ...
            my $builder = Brocken::Lindsay::IR::Builder->new();
            $builder->position_at_end( $func->append_block('entry') );
            $builder->build_ret( $builder->build_ashr(
                Brocken::Lindsay::IR::Constant->new( type => Brocken::Lindsay::IR::Type::i128(), value => 84 ),
                Brocken::Lindsay::IR::Constant->new( type => Brocken::Lindsay::IR::Type::i64(), value => 1 ),
                '%r'
            ) );
...
subtest 'Jenny::Codegen i128 ICmp (Cross-Platform)' => sub {
    ...
    SKIP: {
        ...
        for my $tc (
            ...
            my $f_block = $func->append_block('if.else');
            $builder->position_at_end($entry);
            my $cond = $builder->build_icmp( $pred, Brocken::Lindsay::IR::Constant->new( type => Brocken::Lindsay::IR::Type::i128(), value => $a ), Brocken::Lindsay::IR::Constant->new( type => Brocken::Lindsay::IR::Type::i128(), value => $b ), '%cmp' );
            $builder->build_cond_br( $cond, $t_block, $f_block );
            $builder->position_at_end($t_block);
            $builder->build_ret( Brocken::Lindsay::IR::Constant->new( type => Brocken::Lindsay::IR::Type::i128(), value => 42 ) );
            $builder->position_at_end($f_block);
