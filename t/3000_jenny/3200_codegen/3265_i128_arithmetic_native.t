...
# Test 1: constant i128 return (42)
{
    ...
    my $t1      = time;
    diag( 'Test 1 constant: emit=' . int( ( $t1 - $t0 ) * 1000 + 0.5 ) . 'ms bytes=' . length($bytes) );
...
SKIP: {
        ...
        my $t3 = time;
        diag( 'Test 1 constant: exec=' . int( ( $t3 - $t2 ) * 1000 + 0.5 ) . 'ms total=' . int( ( $t3 - $t0 ) * 1000 + 0.5 ) . 'ms' );
    }
}
...
# Test 2: i128 add (40 + 2 = 42)
{
    ...
    my $bytes   = $codegen->emit_function($func);
    my $t1      = time;
    diag( 'Test 2 add: emit=' . int( ( $t1 - $t0 ) * 1000 + 0.5 ) . 'ms bytes=' . length($bytes) );
...
SKIP: {
        ...
        my $t3 = time;
        diag( 'Test 2 add: exec=' . int( ( $t3 - $t2 ) * 1000 + 0.5 ) . 'ms total=' . int( ( $t3 - $t0 ) * 1000 + 0.5 ) . 'ms' );
    }
}
...
# Test 3: i128 shl (21 << 1 = 42)
{
    ...
    my $bytes   = $codegen->emit_function($func);
    my $t1      = time;
    diag( 'Test 3 shl: emit=' . int( ( $t1 - $t0 ) * 1000 + 0.5 ) . 'ms bytes=' . length($bytes) );
...
SKIP: {
        ...
        my $t3 = time;
        diag( 'Test 3 shl: exec=' . int( ( $t3 - $t2 ) * 1000 + 0.5 ) . 'ms total=' . int( ( $t3 - $t0 ) * 1000 + 0.5 ) . 'ms' );
    }
}
...
# Test 3b: i128 mul (21 * 2 = 42)
{
    ...
    my $bytes   = $codegen->emit_function($func);
    my $t1      = time;
    diag( 'Test 3b mul: emit=' . int( ( $t1 - $t0 ) * 1000 + 0.5 ) . 'ms bytes=' . length($bytes) );
...
SKIP: {
        ...
        my $t3 = time;
        diag( 'Test 3b mul: exec=' . int( ( $t3 - $t2 ) * 1000 + 0.5 ) . 'ms total=' . int( ( $t3 - $t0 ) * 1000 + 0.5 ) . 'ms' );
    }
}
...
# Test 4: i128 lshr (84 >> 1 = 42)
{
    ...
    my $bytes   = $codegen->emit_function($func);
    my $t1      = time;
    diag( 'Test 4 lshr: emit=' . int( ( $t1 - $t0 ) * 1000 + 0.5 ) . 'ms bytes=' . length($bytes) );
...
SKIP: {
        ...
        my $t3 = time;
        diag( 'Test 4 lshr: exec=' . int( ( $t3 - $t2 ) * 1000 + 0.5 ) . 'ms total=' . int( ( $t3 - $t0 ) * 1000 + 0.5 ) . 'ms' );
    }
}
...
# Test 5: i128 ashr (84 >> 1 = 42)
{
    ...
    my $bytes   = $codegen->emit_function($func);
    my $t1      = time;
    diag( 'Test 5 ashr: emit=' . int( ( $t1 - $t0 ) * 1000 + 0.5 ) . 'ms bytes=' . length($bytes) );
...
SKIP: {
        ...
        my $t3 = time;
        diag( 'Test 5 ashr: exec=' . int( ( $t3 - $t2 ) * 1000 + 0.5 ) . 'ms total=' . int( ( $t3 - $t0 ) * 1000 + 0.5 ) . 'ms' );
    }
}
...
# Test 6: i128 div (42 / 2 = 21)
{
    ...
    my $bytes   = $codegen->emit_function($func);
    my $t1      = time;
    diag( 'Test 6 div: emit=' . int( ( $t1 - $t0 ) * 1000 + 0.5 ) . 'ms bytes=' . length($bytes) );
...
SKIP: {
        ...
        my $t3 = time;
        diag( 'Test 6 div: exec=' . int( ( $t3 - $t2 ) * 1000 + 0.5 ) . 'ms total=' . int( ( $t3 - $t0 ) * 1000 + 0.5 ) . 'ms' );
    }
}
...
# Test 7: i128 rem (21 % 10 = 1)
{
    ...
    my $bytes   = $codegen->emit_function($func);
    my $t1      = time;
    diag( 'Test 7 rem: emit=' . int( ( $t1 - $t0 ) * 1000 + 0.5 ) . 'ms bytes=' . length($bytes) );
...
SKIP: {
        ...
        my $t3 = time;
        diag( 'Test 7 rem: exec=' . int( ( $t3 - $t2 ) * 1000 + 0.5 ) . 'ms total=' . int( ( $t3 - $t0 ) * 1000 + 0.5 ) . 'ms' );
    }
}
...
# Test 8: i128 add with carry chain (-1 + 43 = 42)
{
    ...
    my $bytes   = $codegen->emit_function($func);
    my $t1      = time;
    diag( 'Test 8 carry add: emit=' . int( ( $t1 - $t0 ) * 1000 + 0.5 ) . 'ms bytes=' . length($bytes) );
...
SKIP: {
        ...
        my $t3 = time;
        diag( 'Test 8 carry add: exec=' . int( ( $t3 - $t2 ) * 1000 + 0.5 ) . 'ms total=' . int( ( $t3 - $t0 ) * 1000 + 0.5 ) . 'ms' );
    }
}
...
# Test 9: i128 sub with borrow (5 - 3 = 2)
{
    ...
    my $bytes   = $codegen->emit_function($func);
    my $t1      = time;
    diag( 'Test 9 sub: emit=' . int( ( $t1 - $t0 ) * 1000 + 0.5 ) . 'ms bytes=' . length($bytes) );
...
SKIP: {
        ...
        my $t3 = time;
        diag( 'Test 9 sub: exec=' . int( ( $t3 - $t2 ) * 1000 + 0.5 ) . 'ms total=' . int( ( $t3 - $t0 ) * 1000 + 0.5 ) . 'ms' );
    }
}
...
# Test 9b: i128 sub with borrow (0 - 1 = -1)
{
    ...
    my $bytes   = $codegen->emit_function($func);
    my $t1      = time;
    diag( 'Test 9b borrow sub: emit=' . int( ( $t1 - $t0 ) * 1000 + 0.5 ) . 'ms bytes=' . length($bytes) );
...
SKIP: {
        ...
        my $t3 = time;
        diag( 'Test 9b borrow sub: exec=' . int( ( $t3 - $t2 ) * 1000 + 0.5 ) . 'ms total=' . int( ( $t3 - $t0 ) * 1000 + 0.5 ) . 'ms' );
    }
}
...
# Test 10: i128 mul (7 * 6 = 42)
{
    ...
    my $bytes   = $codegen->emit_function($func);
    my $t1      = time;
    diag( 'Test 10 mul: emit=' . int( ( $t1 - $t0 ) * 1000 + 0.5 ) . 'ms bytes=' . length($bytes) );
...
SKIP: {
        ...
        my $t3 = time;
        diag( 'Test 10 mul: exec=' . int( ( $t3 - $t2 ) * 1000 + 0.5 ) . 'ms total=' . int( ( $t3 - $t0 ) * 1000 + 0.5 ) . 'ms' );
    }
}
...
# Test 11: i128 div (100 / 3 = 33)
{
    ...
    my $bytes   = $codegen->emit_function($func);
    my $t1      = time;
    diag( 'Test 11 div: emit=' . int( ( $t1 - $t0 ) * 1000 + 0.5 ) . 'ms bytes=' . length($bytes) );
...
SKIP: {
        ...
        my $t3 = time;
        diag( 'Test 11 div: exec=' . int( ( $t3 - $t2 ) * 1000 + 0.5 ) . 'ms total=' . int( ( $t3 - $t0 ) * 1000 + 0.5 ) . 'ms' );
    }
}
...
# Test 12: i128 rem (100 % 7 = 2)
{
    ...
    my $bytes   = $codegen->emit_function($func);
    my $t1      = time;
    diag( 'Test 12 rem: emit=' . int( ( $t1 - $t0 ) * 1000 + 0.5 ) . 'ms bytes=' . length($bytes) );
...
SKIP: {
        ...
        my $t3 = time;
        diag( 'Test 12 rem: exec=' . int( ( $t3 - $t2 ) * 1000 + 0.5 ) . 'ms total=' . int( ( $t3 - $t0 ) * 1000 + 0.5 ) . 'ms' );
    }
}
...
# Test 13: i128 sub with negative rhs (5 - (-1) = 6)
{
    ...
    my $bytes   = $codegen->emit_function($func);
    my $t1      = time;
    diag( 'Test 13 sub neg: emit=' . int( ( $t1 - $t0 ) * 1000 + 0.5 ) . 'ms bytes=' . length($bytes) );
...
SKIP: {
        ...
        my $t3 = time;
        diag( 'Test 13 sub neg: exec=' . int( ( $t3 - $t2 ) * 1000 + 0.5 ) . 'ms total=' . int( ( $t3 - $t0 ) * 1000 + 0.5 ) . 'ms' );
    }
}
...
# Test 14: i128 add with carry through hi ((-1) + 2 = 1)
{
    ...
    my $bytes   = $codegen->emit_function($func);
    my $t1      = time;
    diag( 'Test 14 carry hi: emit=' . int( ( $t1 - $t0 ) * 1000 + 0.5 ) . 'ms bytes=' . length($bytes) );
...
SKIP: {
        ...
        my $t3 = time;
        diag( 'Test 14 carry hi: exec=' . int( ( $t3 - $t2 ) * 1000 + 0.5 ) . 'ms total=' . int( ( $t3 - $t0 ) * 1000 + 0.5 ) . 'ms' );
    }
}
...
# Test 15: i128 and ((-1) & 42 = 42)
{
    ...
    my $bytes   = $codegen->emit_function($func);
    my $t1      = time;
    diag( 'Test 15 and: emit=' . int( ( $t1 - $t0 ) * 1000 + 0.5 ) . 'ms bytes=' . length($bytes) );
...
SKIP: {
        ...
        my $t3 = time;
        diag( 'Test 15 and: exec=' . int( ( $t3 - $t2 ) * 1000 + 0.5 ) . 'ms total=' . int( ( $t3 - $t0 ) * 1000 + 0.5 ) . 'ms' );
    }
}
...
# Test 16: i128 or (0 | 42 = 42)
{
    ...
    my $bytes   = $codegen->emit_function($func);
    my $t1      = time;
    diag( 'Test 16 or: emit=' . int( ( $t1 - $t0 ) * 1000 + 0.5 ) . 'ms bytes=' . length($bytes) );
...
SKIP: {
        ...
        my $t3 = time;
        diag( 'Test 16 or: exec=' . int( ( $t3 - $t2 ) * 1000 + 0.5 ) . 'ms total=' . int( ( $t3 - $t0 ) * 1000 + 0.5 ) . 'ms' );
    }
}
...
# Test 17: i128 xor (42 xor 0 = 42)
{
    ...
    my $bytes   = $codegen->emit_function($func);
    my $t1      = time;
    diag( 'Test 17 xor: emit=' . int( ( $t1 - $t0 ) * 1000 + 0.5 ) . 'ms bytes=' . length($bytes) );
...
SKIP: {
        ...
        my $t3 = time;
        diag( 'Test 17 xor: exec=' . int( ( $t3 - $t2 ) * 1000 + 0.5 ) . 'ms total=' . int( ( $t3 - $t0 ) * 1000 + 0.5 ) . 'ms' );
    }
}
...
# Test 18: signed i128 div (-42 / 2 = -21)
{
    ...
    my $bytes   = $codegen->emit_function($func);
    my $t1      = time;
    diag( 'Test 18 sdiv1: emit=' . int( ( $t1 - $t0 ) * 1000 + 0.5 ) . 'ms bytes=' . length($bytes) );
...
SKIP: {
        ...
        my $t3 = time;
        diag( 'Test 18 sdiv1: exec=' . int( ( $t3 - $t2 ) * 1000 + 0.5 ) . 'ms total=' . int( ( $t3 - $t0 ) * 1000 + 0.5 ) . 'ms' );
    }
}
...
# Test 19: signed i128 div (42 / -2 = -21)
{
    ...
    my $bytes   = $codegen->emit_function($func);
    my $t1      = time;
    diag( 'Test 19 sdiv2: emit=' . int( ( $t1 - $t0 ) * 1000 + 0.5 ) . 'ms bytes=' . length($bytes) );
...
SKIP: {
        ...
        is( $exit, 235, 'Native i128 signed div (42/-2) returned 235 (-21) on ' . $platform->friendly );
        my $t3 = time;
        diag( 'Test 19 sdiv2: exec=' . int( ( $t3 - $t2 ) * 1000 + 0.5 ) . 'ms total=' . int( ( $t3 - $t0 ) * 1000 + 0.5 ) . 'ms' );
    }
}
...
# Test 20: signed i128 div (-42 / -2 = 21)
{
    ...
    my $bytes   = $codegen->emit_function($func);
    my $t1      = time;
    diag( 'Test 20 sdiv3: emit=' . int( ( $t1 - $t0 ) * 1000 + 0.5 ) . 'ms bytes=' . length($bytes) );
...
SKIP: {
        ...
        my $t3 = time;
        diag( 'Test 20 sdiv3: exec=' . int( ( $t3 - $t2 ) * 1000 + 0.5 ) . 'ms total=' . int( ( $t3 - $t0 ) * 1000 + 0.5 ) . 'ms' );
    }
}
...
# Test 21: signed i128 rem (-42 % 5 = -2, exit 254)
{
    ...
    my $bytes   = $codegen->emit_function($func);
    my $t1      = time;
    diag( 'Test 21 srem: emit=' . int( ( $t1 - $t0 ) * 1000 + 0.5 ) . 'ms bytes=' . length($bytes) );
...
SKIP: {
        ...
        my $t3 = time;
        diag( 'Test 21 srem: exec=' . int( ( $t3 - $t2 ) * 1000 + 0.5 ) . 'ms total=' . int( ( $t3 - $t0 ) * 1000 + 0.5 ) . 'ms' );
    }
}
done_testing;
