...
subtest 'Binary and unary operations' => sub {
    ...
    my $bin  = Brocken::AST::Expr::BinOp->new( left => $int1, op => '+', right => $int2, );
    is $bin->op,           '+',   'BinOp op +';
...
subtest 'If statement' => sub {
    ...
    my $if    = Brocken::AST::Stmt::If->new( condition => $cond, then_block => $block, );
    ...
    my $if_else = Brocken::AST::Stmt::If->new( condition => $cond, then_block => $block, else_block => $block, );
    ok defined( $if_else->else_block ), 'If with else_block';
};
subtest 'While statement' => sub {
    my $body  = Brocken::AST::Stmt::Block->new( statements => [] );
    my $while = Brocken::AST::Stmt::While->new( condition  => Brocken::AST::Expr::IntLiteral->new( value => 1, type => 'Int' ), body => $body, );
