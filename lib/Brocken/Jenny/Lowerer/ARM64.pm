        ...
        for my $block ( $ir_func->blocks->@* ) {
            ...
            for my $inst ( $block->instructions->@* ) {
                ...
                if ( $opcode eq 'add' ||
                    ...
                    if ( $inst->type && $inst->type->kind eq 'float' ) {
                        ...
                        if ($is_i128) {
                            ...
                            $mbb->add_instruction(
                                Brocken::Jenny::MIR::MachineInstruction->new(
                                    opcode   => 'mv',
                                    operands => [ $borrow, $zero ],
                                    comment  => 'i128 neg borrow = 0'
                                )
                            );
                            $mbb->add_instruction(
                                Brocken::Jenny::MIR::MachineInstruction->new(
                                    opcode   => 'sltu',
                                    operands => [ $borrow, $lo_src ],
                                    comment  => 'i128 neg borrow = (0 < lo_src)'
                                )
                            );
                            $mbb->add_instruction(
                                Brocken::Jenny::MIR::MachineInstruction->new(
                                    opcode   => 'mv',
                                    operands => [ $lo_dst, $zero ],
                            ...
                            if ( $val->isa('Brocken::Lindsay::IR::RodataRef') ) {
                                ...
                                $mbb->add_instruction(
                                    Brocken::Jenny::MIR::MachineInstruction->new(
                                        ...
                                        operands => [ $mem, $self->_lower_opnd_wide( $val, $store_type ) ],
                                        comment  => 'store'
                                    )
                                );
                            }
                    ...
                    if ( $inst->base_type->kind eq 'struct' ) {
                    ...
                    elsif ( $indices[0]->isa('Brocken::Lindsay::IR::Constant') ) {
                        my $scale  = $inst->base_type->bits / 8;
                        my $offset = $indices[0]->value * $scale;
                        $mbb->add_instruction(
                            Brocken::Jenny::MIR::MachineInstruction->new(
                                opcode   => 'add',
                                operands => [ $dst, Brocken::Jenny::MIR::MachineOperand->new( kind => 'imm', value => $offset ) ],
                        ...
                        $mbb->add_instruction(
                            Brocken::Jenny::MIR::MachineInstruction->new(
                                opcode   => 'add',
                                operands => [ $dst, Brocken::Jenny::MIR::MachineOperand->new( kind => 'virt_reg', value => $indices[0]->name ) ],
