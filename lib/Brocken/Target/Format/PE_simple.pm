...
class Brocken::Target::Format::PE : isa(Brocken::Target::Format) {
    ...
    method write_bin ( $filename, $text, $data, $arch, $os = 'win64', $edata = undef ) {
        ...
        my $idata_raw
            = pack( 'L< L< L< L< L<', $idata_rva + 16, 0, 0, $idata_rva + 32, $idata_rva ) .
            ( pack( 'L<', 0 ) x 5 ) .
        ...
        my $coff = pack( 'S< S< L< L< L< S< S<', $machine, $num_sections, time(), 0, 0, 224, $characteristics );
        ...
        my $opt = pack(
            ...
            0,      $text_rva, $text_rva, $image_base, $sa, $fa, 6, 0, 0, 0, 6, 0, 0, $size_of_image, $fa, 0, 3, 0x8140, 0x100000, 0x1000, 0x100000,
            0x1000, 0,         16
            ) .
            $data_dirs;
        ...
        # .text section
        $sections .= pack( 'a8 L< L< L< L< L< L< L< S< S< L<', '.text', length($text), $text_rva, length($text_padded), 512, 0, 0, 0, 0, 0x60000020 );

        # .data section
        $sections .= pack(
            'a8 L< L< L< L< L< L< L< S< S< L<',
            '.data', length($data), $data_rva, length($data_padded), 512 + length($text_padded),
            0,       0,             0,         0,                    0xC0000040
        );
        ...
        if ($is_dll) {
            ...
            $sections
                .= pack( 'a8 L< L< L< L< L< L< L< S< S< L<', '.rdata', $rdata_size, $rdata_rva, $rdata_size, $rdata_file, 0, 0, 0, 0, 0x40000040 );
        }
        else {
            $sections .= pack(
                'a8 L< L< L< L< L< L< L< S< S< L<',
                '.idata', length($idata_raw), $idata_rva, length($idata_padded), 512 + length($text_padded) + length($data_padded),
                0,        0,                  0,          0,                     0xC0000040
            );
        }
