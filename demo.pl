...
package Pulse::Format::MachO { }

package Pulse::Format::ELF {

    sub write_bin {
        my ( $class, $filename, $text, $data ) = @_;
        my $base        = 0x400000;
        my $text_off    = 0x1000;
        my $data_off    = 0x2000;
        ...
        my $elf_hdr
            = pack( 'A4 C C C C C x7 S S L Q Q Q L S S S S S S', "\x7fELF", 2, 1, 1, 0, 0, 2, 62, 1, $base + $text_off, 0x40, 0, 0, 64, 56, 2, 0, 0,
            0 );
...
package Pulse::Format::PE {
    ...
    sub write_bin {
        ...
        my $file_hdr     = pack( 'S< S< L< L< L< S< S<', 0x8664, 3, time(), 0, 0, 240, 0x0022 );
        my $opt_hdr      = pack(
...
class Pulse::Compiler {
    field $as     : reader = Pulse::Emit::X64->new();
    field $target : reader : param //= ( $^O eq 'MSWin32' ? 'win64' : 'linux' );
    field $data   : reader = '';
    #
    field $label_count = 0;
    ...
    method emit_alloc ( $size_bytes, $dest_reg ) {
        ...
        $as->mov_reg( 'rdx', 'rax' );                # RAX holds new object pointer
        $as->jmp($l_end);
        ...
        # Move result to user's requested register
    ...
    method emit_try_catch ( $try_cb, $catch_cb ) {
        ...
        # Note: Throw restores RSP and RBP automatically, and puts err code in RAX.
        $catch_cb->($as);
        $as->mark_label($l_end);
    }
    ...
    # $frame_size is the amount of stack space allocated by `sub_imm('rsp', X)`
    method emit_tail_call ( $target_label, $frame_size = 0 ) {

        # Unwind the current function's stack frame
        $as->add_imm( 'rsp', $frame_size ) if $frame_size > 0;

        # If we were doing standard base-pointer framing, we'd also do: $as->pop_reg('rbp');
        # Jump directly to the target.
        # When the target calls `ret`, it will return to our caller's caller!
        $as->jmp($target_label);
    }
}
#
my $target = shift @ARGV // ( $^O eq 'MSWin32' ? 'win64' : 'linux' );
say 'Targeting OS: ' . $target;
my $p  = Pulse::Compiler->new( target => $target );
my $as = $p->as;
#
# Windows ABI: Setup 32 bytes shadow space
# + 8 bytes for Arg5 + 8 bytes for Arg6 + 8 bytes alignment = 56
$as->sub_imm( 'rsp', 56 ) if $target eq 'win64';
...
$as->add_imm( 'rbx', 1 );
$as->cmp_reg_imm( 'rbx', 4 );
$as->jcc( 0x0C, 'loop' );    # JL (Jump Less than 4)
$p->print_str("Done! Exiting with status 42.\n");
$p->exit_proc(42);
#
$as->resolve();
if ( $target eq 'linux' ) {
    Pulse::Format::ELF->write_bin( 'pulse_output', $as->code(), $p->data );
    say 'Generated Linux ELF: ./pulse_output';
    say 'Exit code: ' . ( system('./pulse_output') >> 8 );
}
else {
    Pulse::Format::PE->write_bin( 'pulse_output.exe', $as->code(), $p->data );
    say 'Generated Windows PE: .\pulse_output.exe';
    say 'Exit code: ' . ( system('./pulse_output.exe') >> 8 );
}
