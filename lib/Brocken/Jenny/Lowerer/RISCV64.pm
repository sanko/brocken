        ...
        for my $block ( $ir_func->blocks->@* ) {
            ...
            for my $inst ( $block->instructions->@* ) {
                ...
                if ( $opcode eq 'add' ||
                    ...
                    if ( $inst->type && $inst->type->kind eq 'float' ) {
                        ...
                        if ( $is_i128 && ( $opcode eq 'min' || $opcode eq 'max' ) ) {
                            ...
                            $mbb->add_instruction(
                                Brocken::Jenny::MIR::MachineInstruction->new(
                                    opcode   => 'slt',
                                    operands => [ $mask, $hi_lhs, $hi_rhs ],
                                    comment  => 'i128 minmax hi slt'
                                )
                            );
                            $mbb->add_instruction(
                                Brocken::Jenny::MIR::MachineInstruction->new(
                            ...
                            $mbb->add_instruction(
                                Brocken::Jenny::MIR::MachineInstruction->new(
                                    opcode   => 'sltiu',
                                    operands => [ $t0, Brocken::Jenny::MIR::MachineOperand->new( kind => 'imm', value => 1 ) ],
                                    comment  => 'i128 minmax hi seqz'
                                )
                            );
                            $mbb->add_instruction(
                                Brocken::Jenny::MIR::MachineInstruction->new(
                                    opcode   => 'sltu',
                                    operands => [ $t1, $lo_lhs, $lo_rhs ],
                                    comment  => 'i128 minmax lo sltu'
                                )
                            );
                            $mbb->add_instruction(
                                Brocken::Jenny::MIR::MachineInstruction->new(
                    ...
                    my $val_lowered
                        = ( $val->isa('Brocken::Lindsay::IR::RodataRef') || $val->isa('Brocken::Lindsay::IR::Constant') ) ?
                        $self->_materialize( $mbb, $val ) :
                        $self->_lower_opnd($val);
                    $mbb->add_instruction(
                        Brocken::Jenny::MIR::MachineInstruction->new(
                            opcode   => 'mov',
                            operands => [ $reg, $val_lowered ],
