...
class Brocken::Target::Format::PE : isa(Brocken::Target::Format) {
    ...
    method write_bin ( $filename, $text, $data, $arch, $os = 'win64', $edata = undef ) {
        ...
        my $coff = pack('S< S< L< L< L< S< S<',
            ($arch eq 'arm64' ? 0xAA64 : 0x8664),
            $num_sections,
            time(),
            0, 0, 224,
            $is_dll ? 0x2102 : 0x0102
        );
        ...
        $sections = pack('a8 L< L< L< L< L< L< S< L<',
            '.text', length($text), $sa, length($text_padded), 0x200, 0, 0, 0x60000020);
        $sections .= pack('a8 L< L< L< L< L< L< S< L<',
            '.data', length($data), $sa*2, length($data_padded), 0x400, 0, 0, 0xC0000040);
        $sections .= pack('a8 L< L< L< L< L< L< S< L<',
            '.idata', length($idata), 0x3000, length($idata_padded), 0x600, 0, 0, 0xC0000040);
        if ($is_dll) {
            $sections .= pack('a8 L< L< L< L< L< L< S< L<',
                '.edata', length($edata), 0x4000, length($edata_padded), 0x800, 0, 0, 0x40000040);
        }
    ...
    method write_lib ( $filename, $text, $data, $arch, $os, $exports ) {
        ...
        my $edata = pack('L< L< S< S< L< L< L< L< L< L< L<',
            0, 0, 0, 0,
            $str_base,  # DLL name RVA
            1,  # Ordinal base
            $num, $num,  # Count
            $base_rva + 40,  # EAT
