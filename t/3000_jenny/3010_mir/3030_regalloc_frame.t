...
subtest 'insert_caller_save_code_float' => sub {
    ...
    my $bb    = Brocken::Jenny::MIR::MachineBasicBlock->new( name => 'entry' );
    $mf->add_block($bb);
    my $vreg_a      = Brocken::Jenny::MIR::MachineOperand->new( kind => 'virt_reg', value => '%a', type => Brocken::Lindsay::IR::Type::f32() );
    my $imm_1       = Brocken::Jenny::MIR::MachineOperand->new( kind => 'imm',      value => 1.0,  type => Brocken::Lindsay::IR::Type::f32() );
    my $call_target = Brocken::Jenny::MIR::MachineOperand->new( kind => 'func',     value => 'helper' );
    $bb->add_instruction( Brocken::Jenny::MIR::MachineInstruction->new( opcode => 'fadd',      operands => [ $vreg_a, $imm_1 ] ) );
    $bb->add_instruction( Brocken::Jenny::MIR::MachineInstruction->new( opcode => 'call_func', operands => [$call_target] ) );
    $bb->add_instruction( Brocken::Jenny::MIR::MachineInstruction->new( opcode => 'ret',       operands => [] ) );
    $alloc->insert_caller_save_code( $mf, [ 'xmm0', 'xmm1' ], 'rsp', 1 );
    my @insts = $bb->instructions->@*;
    is scalar(@insts),    7,           'added 4 fsave/frestore around call (2 regs * 2)';
    is $insts[1]->opcode, 'fstore',    'float save uses fstore';
    is $insts[2]->opcode, 'fstore',    'second float save';
    is $insts[3]->opcode, 'call_func', 'call in middle';
    is $insts[4]->opcode, 'fload',     'float restore uses fload';
    is $insts[5]->opcode, 'fload',     'second float restore';
    is $insts[6]->opcode, 'ret',       'ret preserved';
};
...
subtest 'x86_64 leaf vs non-leaf prologue' => sub {
    ...
    # Modern: both leaf and non-leaf use standard frame pointers!
    # Encoding for standard prologue: 55 (push rbp), 48 89 E5 (mov rbp, rsp)
    is( substr( $leaf_bytes,    0, 4 ), pack( 'C*', 0x55, 0x48, 0x89, 0xE5 ), 'leaf pro: push rbp; mov rbp, rsp' );
    is( substr( $nonleaf_bytes, 0, 4 ), pack( 'C*', 0x55, 0x48, 0x89, 0xE5 ), 'non-leaf pro: push rbp; mov rbp, rsp' );
...
SKIP: {
        ...
        run_exec( $output_file, expected_exit => 105, platform => $platform, keep => 1, name => 'leaf function returned sum of 1..14 = 105' );
        unlink $output_file;
    }
};
done_testing;
