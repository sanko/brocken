...
class Brocken::Parser {
    ...
    method parse () {
        ...
        return Brocken::AST::Stmt::Program->new( statements => \@stmts );
    }
    ...
    method _parse_stmt () {
        ...
        if ( $tok->type eq 'KEYWORD' ) {
            ...
            if ( $val eq 'return' ) {
                ...
                if ( $tok && $tok->type ne ';' && $tok->type ne 'KEYWORD' ) {
                    ...
                    $stmt = Brocken::AST::Stmt::Return->new( expr => $expr );
                }
                else {
                    $stmt = Brocken::AST::Stmt::Return->new();
                }
                return $self->_finish_stmt_with_modifier($stmt);
            }
            if ( $val =~ /^(last|next|redo)$/ ) {
                $self->_next();
                my $class = "Brocken::AST::Stmt::" . ucfirst($val);
                my $stmt  = $class->new();
                $self->_expect(';');
                return $stmt;
            }
            if ( $val eq 'spawn_thread' ) {
                ...
                my $stmt = Brocken::AST::Expr::Call->new( name => 'spawn_thread', args => [$sub] );
                return $self->_finish_stmt_with_modifier($stmt);
            }
        }
        ...
        if ( $tok && $tok->type eq 'IDENT' ) {
            ...
            my $stmt = Brocken::AST::Expr::Call->new( name => $name, args => \@args );
            return $self->_finish_stmt_with_modifier($stmt);
        }
    ...
    method _finish_stmt_with_modifier ($stmt) {
        if ( $tok && $tok->type eq 'KEYWORD' && $tok->value =~ /^(if|unless|while|until)$/ ) {
            ...
            my $block = Brocken::AST::Stmt::Block->new( statements => [$stmt] );
            if ( $mod eq 'if' ) {
                return Brocken::AST::Stmt::If->new( condition => $cond, then_block => $block );
            }
            if ( $mod eq 'unless' ) {
                my $not_cond = Brocken::AST::Expr::UnaryOp->new( op => '!', expr => $cond );
                return Brocken::AST::Stmt::If->new( condition => $not_cond, then_block => $block );
            }
            if ( $mod eq 'while' ) {
                return Brocken::AST::Stmt::While->new( condition => $cond, body => $block );
            }
            if ( $mod eq 'until' ) {
                my $not_cond = Brocken::AST::Expr::UnaryOp->new( op => '!', expr => $cond );
                return Brocken::AST::Stmt::While->new( condition => $not_cond, body => $block );
            }
        }
        if ( $tok && $tok->type eq ';' ) {
            $self->_next();
    ...
    method _parse_if_stmt () {
        ...
        if ( $tok && $tok->type eq 'KEYWORD' && $tok->value eq 'elsif' ) {
            ...
            $else = Brocken::AST::Stmt::Block->new( statements => [$inner_if] );
        }
    ...
    method _parse_unless_stmt () {
        ...
        return Brocken::AST::Stmt::If->new( condition => $not_cond, then_block => $then, else_block => $else );
    }
    ...
    method _parse_while_stmt () {
        ...
        return Brocken::AST::Stmt::While->new( condition => $cond, body => $body );
    }
    ...
    method _parse_until_stmt () {
        ...
        $self->_expect(')');
        my $body = $self->_parse_block();
        return Brocken::AST::Stmt::While->new( condition => $not_cond, body => $body );
    }
    ...
    method _parse_my_decl ($keyword) {
        ...
        my %class_map = ( my => 'VarDecl', our => 'OurDecl', state => 'StateDecl' );
        my $class     = "Brocken::AST::Stmt::" . $class_map{$keyword};
        return $class->new( name => $name, type => $type, value => $expr );
    }
    ...
    method _parse_for_stmt () {
        ...
        $self->_expect(')');
        my $body = $self->_parse_block();
        return Brocken::AST::Stmt::For->new( init => $init, condition => $cond, step => $step, body => $body );
    }
    ...
    method _parse_foreach_stmt () {
        ...
        $self->_expect(')');
        my $body = $self->_parse_block();
        return Brocken::AST::Stmt::For->new( var => $var->value, source => $expr, body => $body, is_my => 1 );
    }
    ...
    method _nud ($tkn) {
        ...
        my $line = $tkn->line;
        my $col  = $tkn->col;
        if ( $t eq 'INT' )    { return Brocken::AST::Expr::IntLiteral->new( value => $val, type => 'Int', line => $line, col => $col ) }
        if ( $t eq 'FLOAT' )  { return Brocken::AST::Expr::FloatLiteral->new( value => $val, type => 'Float', line => $line, col => $col ) }
        if ( $t eq 'STRING' ) { return Brocken::AST::Expr::StrLiteral->new( value => $val, type => 'String', line => $line, col => $col ) }
        if ( $t eq 'VAR' ) {
            return Brocken::AST::Expr::Var->new( name => $val, sigil => $tkn->sigil // '$', line => $line, col => $col );
        }
        if ( $t eq 'IDENT' ) {
            if ( $tok && $tok->type eq '(' ) {
                $self->_next();
                my @args = $self->_parse_args();
                return Brocken::AST::Expr::Call->new( name => $val, args => \@args, line => $line, col => $col );
            }
            return Brocken::AST::Expr::Var->new( name => $val, line => $line, col => $col );
        }
        if ( $t eq 'KEYWORD' && $val eq 'sub' ) {
            ...
            my $body = $self->_parse_block();
            return Brocken::AST::OOP::AnonSub->new( params => \@params, body => $body, line => $line, col => $col );
        }
        ...
        if ( $t eq '-' || $t eq '+' || $t eq '!' || $t eq '~' || $t eq 'not' ) {
            ...
            return Brocken::AST::Expr::UnaryOp->new( op => $t, expr => $operand, line => $line, col => $col );
        }
        if ( $t eq '++' || $t eq '--' ) {
    ...
    method _led ( $op_tok, $left, $next_bp ) {
        ...
        if ( $t eq '?' ) {
            ...
            return Brocken::AST::Expr::Ternary->new( cond => $left, then => $then, else => $else, line => $op_tok->line, col => $op_tok->col );
        }

        # Assignment
        if ( $t eq '=' || ( $t =~ /=$/ && $t !~ /^[=!<>]=$/ ) ) {
            die "Can't assign to non-variable at {$op_tok->line:$op_tok->col}" unless $left->isa('Brocken::AST::Expr::Var');
            my $right = $self->_parse_expr($next_bp);
            if ( $t eq '=' ) {
                return Brocken::AST::Stmt::Assignment->new( name => $left->name, value => $right, line => $op_tok->line, col => $op_tok->col );
            }
            my $op  = substr $t, 0, -1;
            my $bin = Brocken::AST::Expr::BinOp->new( left => $left, op => $op, right => $right );
            return Brocken::AST::Stmt::Assignment->new( name => $left->name, value => $bin, line => $op_tok->line, col => $op_tok->col );
        }
        ...
        if ( $t eq '[' ) {
            ...
            $self->_expect(']');
            return Brocken::AST::Expr::IndexExpr->new( source => $left, index => $index, line => $op_tok->line, col => $op_tok->col );
        }
        ...
        if ( $t eq '{' ) {
            ...
            $self->_expect('}');
            return Brocken::AST::Expr::IndexExpr->new( source => $left, index => $key, line => $op_tok->line, col => $op_tok->col );
        }

        # Function call: foo(args)
        if ( $t eq '(' ) {
            my @args = $self->_parse_args();
            return Brocken::AST::Expr::Call->new( name => $val, args => \@args, line => $op_tok->line, col => $op_tok->col );
        }
        ...
        my $right = $self->_parse_expr($next_bp);
        return Brocken::AST::Expr::BinOp->new( left => $left, op => $t, right => $right, line => $op_tok->line, col => $op_tok->col );
    }
}
1;
