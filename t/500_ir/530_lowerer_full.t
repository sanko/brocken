...
subtest 'Expr::NilLiteral' => sub {
    ...
    my $cfg = $lowerer->lower([ wrap_as_stmt(
        Brocken::AST::Expr::NilLiteral->new(value => 'undef', type => 'Undef')
    ) ]);
    my @instr = $cfg->entry_block->instructions;
...
subtest 'Expr::Exists' => sub {
    my $lowerer = Brocken::Compiler::Lowerer->new();
    my $cfg = $lowerer->lower([ wrap_as_stmt(
        Brocken::AST::Expr::Exists->new(
            expr => Brocken::AST::Expr::Var->new(name => 'h')
        )
    ) ]);
    my @instr = $cfg->entry_block->instructions;
...
subtest 'Stmt::Exit (direct)' => sub {
    my $lowerer = Brocken::Compiler::Lowerer->new();
    my $cfg = $lowerer->lower([
        Brocken::AST::Stmt::Exit->new(
            expr => Brocken::AST::Expr::IntLiteral->new(value => 0, type => 'Int')
        )
    ]);
    my @instr = $cfg->entry_block->instructions;
    my $call = (grep { /call exit\(/ } map { $_->to_string } @instr)[0];
